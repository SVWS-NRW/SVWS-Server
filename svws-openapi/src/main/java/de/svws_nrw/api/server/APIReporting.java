...
public class APIReporting {
    ...
    /**
     * Die OpenAPI-Methode für die Erstellung eines Reports im PDF-Format. Je nach übergebenen Parametern wird eine
	 * einzelne PDF-Datei oder eine ZIP-Datei mit einzelnen PDF-Dateien zurückgegeben.
     *
     ...
     *
     * @return Die PDF-Datei (bzw. ZIP-Datei mit einzelnen PDF-Dateien) mit den angeforderten Daten
     */
	@POST
	...
	@Operation(
		...
		description =
			"Erstellt die Wahlbogen für die Laufbahnplanung der gymnasialen Oberstufe zu den Schülern mit der "
	...
	@ApiResponse(
		responseCode = "200",
		description = "Der Report mit den übergebenen Daten wurde erfolgreich erstellt.",
		content = @Content(mediaType = "application/pdf", schema = @Schema(type = "string", format = "binary", description = "Report")))
	@ApiResponse(
		responseCode = "403",
		description = "Der SVWS-Benutzer hat keine Rechte, um den geforderten Report zu erstellen.")
	@ApiResponse(
		responseCode = "404",
		description = "Kein Eintrag zu den übergebenen Daten gefunden.")
	@ApiResponse(
		responseCode = "500",
	...
	public Response pdfReport(
		...
		@RequestBody(
			description = "Die Daten und Einstellungen, mit denen der Report erstellt werden soll.",
			required = true,
			content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ReportingAusgabedaten.class)))
		final ReportingAusgabedaten reportingAusgabedaten,
		@Context final HttpServletRequest request) {
    	return DBBenutzerUtils.runWithTransaction(
			conn ->	new PdfFactory(conn, reportingAusgabedaten).create(),
			request,
			ServerMode.STABLE,
