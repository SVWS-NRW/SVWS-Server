...
public class APIBetrieb {
	...
	@Operation(summary = "Gibt eine Übersicht von allen Betrieben zurück.",
			description = "Erstellt eine Liste aller in der Datenbank vorhandenen Betriebe unter Angabe der ID, der Betriebsart , " // TODO Beschreibung anpassen.
					+ "des Betriebnamens, Kontaktdaten, ob sie in der Anwendung sichtbar bzw. änderbar sein sollen. "
	...
	@Operation(summary = "Erstellt einen neuen Betrieb und gibt den neuen Datensatz zurück.",
			description = "Erstellt einen neuen Betrieb und gibt den neuen Datensatz zurück."
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Erstellen eines Betriebes besitzt.")
	...
	public Response createBetrieb(
			...
			@RequestBody(description = "Der Post für die Betrieb-Daten", required = true, content = @Content(mediaType = MediaType.APPLICATION_JSON,
					schema = @Schema(implementation = BetriebStammdaten.class))) final InputStream is,
	...
	public Response patchBetriebStammdaten(@PathParam("schema") final String schema, @PathParam("id") final long id,
			@RequestBody(description = "Der Patch für die Betrieb-Stammdaten", required = true, content = @Content(mediaType = MediaType.APPLICATION_JSON,
					schema = @Schema(implementation = BetriebStammdaten.class))) final InputStream is,
			@Context final HttpServletRequest request) {
	...
	public Response removeBetrieb(@PathParam("schema") final String schema,
			@RequestBody(description = "Die IDs der Betriebe", required = true, content = @Content(mediaType = MediaType.APPLICATION_JSON,
					array = @ArraySchema(schema = @Schema(implementation = Long.class)))) final List<Long> bids,
			@Context final HttpServletRequest request) {
	...
	@Operation(summary = "Passt die Schüler-Betriebsdaten zu der angegebenen ID an und speichert das Ergebnis in der Datenbank.",
			description = "Passt die Schüler-Betriebsdaten zu der angegebenen ID an und speichert das Ergebnis in der Datenbank. "
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Ändern von Schülerbetreibsdaten besitzt.")
	...
	*/
	...
	@Operation(summary = "Gibt eine Übersicht von allen Betriebansprechpartnern zurück.",
			description = "Erstellt eine Liste aller in der Datenbank vorhandenen Betriebansprechpartner , "
					+ "des Ansprechpartnername, Kontaktdaten, ob sie in der Anwendung sichtbar bzw. änderbar sein sollen. "
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Ansehen von Betriebsansprechpartnern besitzt.")
	...
	@ApiResponse(responseCode = "403", description = "Der SVWS-Benutzer hat keine Rechte, um Betriebdaten anzusehen.")
	@ApiResponse(responseCode = "404", description = "Keine Betrieb-Einträge gefunden")
	public Response getBetriebAnsprechpartner(@PathParam("schema") final String schema, @PathParam("id") final long betrieb_id,
			@Context final HttpServletRequest request) {
	...
	@Operation(summary = "Gibt eine Übersicht von allen Betriebansprechpartnern zurück.",
			description = "Erstellt eine Liste aller in der Datenbank vorhandenen Betriebansprechpartner, des Ansprechpartnername, Kontaktdaten, ob sie "
					+ "in der Anwendung sichtbar bzw. änderbar sein sollen. "
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Ansehen von Betriebsansprechpartnern besitzt.")
	@ApiResponse(responseCode = "200", description = "Eine Liste von Betriebansprechpartnern",
			content = @Content(mediaType = MediaType.APPLICATION_JSON, array = @ArraySchema(schema = @Schema(implementation = BetriebAnsprechpartner.class))))
	@ApiResponse(responseCode = "403", description = "Der SVWS-Benutzer hat keine Rechte, um Betriebdaten anzusehen.")
	...
	@Operation(summary = "Liefert zu der ID des  Betriebanpsrechpartners die zugehörigen Daten..",
			description = "Liest die Daten des Betriebanpsrechpartners zu der angegebenen ID aus der Datenbank und liefert diese zurück. "
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Ansehen vom Betriebanpsrechpartner besitzt.")
	...
	public Response getBetriebAnsprechpartnerdaten(@PathParam("schema") final String schema, @PathParam("id") final long id,
			@Context final HttpServletRequest request) {
	...
	@Operation(summary = "Passt die Betriebanpsrechpartner-Daten zu der angegebenen ID an und speichert das Ergebnis in der Datenbank.",
			description = "Passt die Betriebanpsrechpartner-Daten zu der angegebenen ID an und speichert das Ergebnis in der Datenbank. "
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Ändern vom Betriebanpsrechpartner besitzt.")
	...
	public Response patchBetriebanpsrechpartnerdaten(
			@PathParam("schema") final String schema,
			@PathParam("id") final long id,
			@RequestBody(description = "Der Patch für die Betriebanpsrechpartner-Daten", required = true,
					content = @Content(mediaType = MediaType.APPLICATION_JSON,
							schema = @Schema(implementation = BetriebAnsprechpartner.class))) final InputStream is,
			@Context final HttpServletRequest request) {
	...
	@Operation(summary = "Erstellt einen neuen Betriebansprechpartner und gibt die dazugehörige ID zurück.",
			description = "Erstellt einen neuen Betriebansprechpartner und gibt die dazugehörige ID zurück."
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Erstellen eines Betriebansprechpartners besitzt.")
	...
	public Response removeBetriebansprechpartner(
			@PathParam("schema") final String schema,
			@RequestBody(description = "Die IDs der Betriebsansprechpartner", required = true, content = @Content(mediaType = MediaType.APPLICATION_JSON,
					array = @ArraySchema(schema = @Schema(implementation = Long.class)))) final List<Long> bids,
			@Context final HttpServletRequest request) {
