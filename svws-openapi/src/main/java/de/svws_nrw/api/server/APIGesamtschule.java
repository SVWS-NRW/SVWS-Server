...
public class APIGesamtschule {
	...
	@Operation(summary = "Liefert zu der ID des Schülers die zugehörigen Leistungsdaten des aktuellen Lernabschnittes in Bezug auf die Prognose- bzw. "
			+ "Abschlussberechnung in der Sek I der Gesamtschule.",
	...
	@ApiResponse(responseCode = "200", description = "Die Leistungsdaten des Schülers",
			content = @Content(mediaType = "application/json", schema = @Schema(implementation = GEAbschlussFaecher.class)))
	...
	@Operation(
			summary = "Liefert zu der ID des Schülers die zugehörigen Leistungsdaten des angegegebenen Lernabschnittes in Bezug auf die Prognose- bzw. "
					+ "Abschlussberechnung in der Sek I der Gesamtschule.",
	...
	public Response getGesamtschuleSchuelerPrognosLeistungsdatenFuerAbschnitt(@PathParam("schema") final String schema, @PathParam("id") final long id,
			...
			@Context final HttpServletRequest request) {
		return DBBenutzerUtils.runWithTransaction(conn -> new DataGEAbschlussFaecher(conn).getByAbschnitt(id, abschnittID, false),
				request, ServerMode.STABLE, BenutzerKompetenz.SCHUELER_LEISTUNGSDATEN_ANSEHEN);
	}
	 ...
	 */
	@GET
	@Path("/schueler/{id : \\d+}/prognose_leistungsdaten/quartal")
	@Operation(summary = "Liefert zu der ID des Schülers die zugehörigen Leistungsdaten des aktuellen Lernabschnittes in Bezug auf die Prognose- bzw. "
			+ "Abschlussberechnung in der Sek I der Gesamtschule basierend auf den Quartalsnoten.",
			description = "Liest die Leistungsdaten des aktuellen Lernabschnittes in Bezug auf die Prognose- bzw. Abschlussberechnung in der Sek I "
					+ "der Gesamtschule des Schülers mit der angegebene ID aus der Datenbank und liefert diese zurück. "
					+ "Dabei wird geprüft, ob der SVWS-Benutzer die notwendige Berechtigung zum Ansehen der Leistungsdaten besitzt.")
	@ApiResponse(responseCode = "200", description = "Die Leistungsdaten des Schülers",
	...
	@ApiResponse(responseCode = "403", description = "Der SVWS-Benutzer hat keine Rechte, um die Leistungsdaten anzusehen.")
	@ApiResponse(responseCode = "404", description = "Kein Schüler-Eintrag mit der angegebenen ID gefunden")
	public Response getGesamtschuleSchuelerPrognoseLeistungsdatenQuartal(@PathParam("schema") final String schema, @PathParam("id") final long id,
			@Context final HttpServletRequest request) {
		return DBBenutzerUtils.runWithTransaction(conn -> new DataGEAbschlussFaecher(conn).getForQuartal(id),
				request, ServerMode.STABLE, BenutzerKompetenz.SCHUELER_LEISTUNGSDATEN_ANSEHEN);
	}
	 ...
	 */
	@GET
	@Path("/schueler/{id : \\d+}/prognose_leistungsdaten/quartal/abschnitt/{abschnittID : \\d+}")
	@Operation(
			summary = "Liefert zu der ID des Schülers die zugehörigen Leistungsdaten des angegegebenen Lernabschnittes in Bezug auf die Prognose- bzw. "
					+ "Abschlussberechnung in der Sek I der Gesamtschule.",
			description = "Liest die Leistungsdaten des angegebenen Lernabschnittes in Bezug auf die Prognose- bzw. Abschlussberechnung in der Sek I "
	...
	@ApiResponse(responseCode = "403", description = "Der SVWS-Benutzer hat keine Rechte, um die Leistungsdaten anzusehen.")
	@ApiResponse(responseCode = "404", description = "Kein Schüler-Eintrag mit der angegebenen ID gefunden")
	public Response getGesamtschuleSchuelerPrognosLeistungsdatenQuartalFuerAbschnitt(@PathParam("schema") final String schema, @PathParam("id") final long id,
			@PathParam("abschnittID") final long abschnittID,
			@Context final HttpServletRequest request) {
		return DBBenutzerUtils.runWithTransaction(conn -> new DataGEAbschlussFaecher(conn).getByAbschnitt(id, abschnittID, true),
				request, ServerMode.STABLE, BenutzerKompetenz.SCHUELER_LEISTUNGSDATEN_ANSEHEN);
	}
