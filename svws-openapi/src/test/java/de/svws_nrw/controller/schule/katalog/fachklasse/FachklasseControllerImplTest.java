...
class FachklasseControllerImplTest {

	@Mock
	private FachklasseService fachklasseService;

	private FachklasseControllerImpl fachklasseControllerImpl;

	private MockedStatic<BeanValidator> beanValidatorMock;

	@BeforeEach
	void setUp() {
		fachklasseControllerImpl = new FachklasseControllerImpl(fachklasseService);
	...
	void getAll() {
		...
		assertThat(fachklasseControllerImpl.getAll())
	...
	void create_success() {
		...
		assertThat(fachklasseControllerImpl.create(dto))
				.isInstanceOf(Response.class)
	...
	void create_validationError() {
		...
		assertThatThrownBy(() -> fachklasseControllerImpl.create(dto))
	...
	void delete_success() {
		...
		assertThat(fachklasseControllerImpl.delete(ids))
				.isInstanceOf(Response.class)
				.hasFieldOrPropertyWithValue("status", Response.Status.OK.getStatusCode())
				.extracting(Response::getEntity)
				.asInstanceOf(InstanceOfAssertFactories.LIST)
	...
	void patch_success() {
		...
		assertThat(fachklasseControllerImpl.patch(id, dto))
				.isInstanceOf(Response.class)
				.hasFieldOrPropertyWithValue("status", Response.Status.OK.getStatusCode())
				.extracting(Response::getEntity)
				.isNotNull();
	...
	void patch_validationError() {
		...
		assertThatThrownBy(() -> fachklasseControllerImpl.patch(id, dto))
				.isInstanceOf(ValidationException.class)
