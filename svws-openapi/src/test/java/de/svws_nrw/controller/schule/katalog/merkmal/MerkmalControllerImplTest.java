...
class MerkmalControllerImplTest {

	@Mock
	private MerkmalService merkmalService;

	private MerkmalControllerImpl merkmalControllerImpl;

	private MockedStatic<BeanValidator> beanValidatorMock;

	@BeforeEach
	void setUp() {
		merkmalControllerImpl = new MerkmalControllerImpl(merkmalService);
	...
	void getAll() {
		...
		assertThat(merkmalControllerImpl.getAll())
	...
	void create_success() {
		...
		assertThat(merkmalControllerImpl.create(dto))
				.isInstanceOf(Response.class)
	...
	void create_validationError() {
		...
		assertThatThrownBy(() -> merkmalControllerImpl.create(dto))
	...
	void delete_success() {
		...
		assertThat(merkmalControllerImpl.delete(ids))
				.isInstanceOf(Response.class)
				.hasFieldOrPropertyWithValue("status", Response.Status.OK.getStatusCode())
				.extracting(Response::getEntity)
				.asInstanceOf(InstanceOfAssertFactories.LIST)
	...
	void patch_success() {
		...
		assertThat(merkmalControllerImpl.patch(id, dto))
				.isInstanceOf(Response.class)
				.hasFieldOrPropertyWithValue("status", Response.Status.OK.getStatusCode())
				.extracting(Response::getEntity)
				.isNotNull()
	...
	void patch_validationError() {
		...
		assertThatThrownBy(() -> merkmalControllerImpl.patch(id, dto))
				.isInstanceOf(ValidationException.class)
