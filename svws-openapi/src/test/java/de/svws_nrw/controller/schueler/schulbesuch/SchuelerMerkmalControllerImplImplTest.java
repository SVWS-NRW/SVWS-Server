...
class SchuelerMerkmalControllerImplImplTest {

	@Mock
	private SchuelerMerkmalService service;

	private SchuelerMerkmalControllerImpl controller;

	private MockedStatic<BeanValidator> beanValidatorMock;

	@BeforeEach
	void setUp() {
		controller = new SchuelerMerkmalControllerImpl(service);
	...
	void create_success() {
		final var dto = mock(SchuelerMerkmalCreateRequest.class);
		final var created = mock(SchuelerSchulbesuchMerkmal.class);
		...
		assertThat(controller.create(dto))
				...
				.isNotNull()
				.satisfies(entity -> assertThat(entity).isInstanceOf(SchuelerSchulbesuchMerkmal.class));
	...
	void patch_success() {
		final var id = 1L;
		final var dto = mock(SchuelerMerkmalPatchRequest.class);
		final var patched = mock(SchuelerSchulbesuchMerkmal.class);
