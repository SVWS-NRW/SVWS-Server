...
describe("APIKlassen Tests", () => {
	describe.each([{ schema: "GymAbi01" }])('gegen %s', ({ schema }) => {
		...
		test('GET: Alle Klassen zum Schuljahresabschnitt 1', async () => {
			const result = await handleRequest(api, api.getListKlassenDatenBySchuljahresabschnitt, schema, 1);
		...
		test('GET: Alle Klassen zu einem Schuljahresabschnitt, der nicht existiert', async () => {
			const result = await handleRequest(api, api.getListKlassenDatenBySchuljahresabschnitt, schema, 0);
