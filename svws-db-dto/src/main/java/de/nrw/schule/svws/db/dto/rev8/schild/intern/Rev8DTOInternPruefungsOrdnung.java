...
@NamedQuery(name="Rev8DTOInternPruefungsOrdnung.primaryKeyQuery", query="SELECT e FROM Rev8DTOInternPruefungsOrdnung e WHERE e.PO_Schulform = ?1 AND e.PO_Krz = ?2 AND e.PO_SGL = ?3")
@NamedQuery(name="Rev8DTOInternPruefungsOrdnung.all.migration", query="SELECT e FROM Rev8DTOInternPruefungsOrdnung e WHERE e.PO_Schulform IS NOT NULL AND e.PO_Krz IS NOT NULL AND e.PO_SGL IS NOT NULL")
