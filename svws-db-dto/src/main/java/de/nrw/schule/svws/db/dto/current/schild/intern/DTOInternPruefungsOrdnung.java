...
@NamedQuery(name="DTOInternPruefungsOrdnung.primaryKeyQuery", query="SELECT e FROM DTOInternPruefungsOrdnung e WHERE e.PO_Krz = ?1 AND e.PO_Schulform = ?2 AND e.PO_SGL = ?3")
@NamedQuery(name="DTOInternPruefungsOrdnung.all.migration", query="SELECT e FROM DTOInternPruefungsOrdnung e WHERE e.PO_Krz IS NOT NULL AND e.PO_Schulform IS NOT NULL AND e.PO_SGL IS NOT NULL")
