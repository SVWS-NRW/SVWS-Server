...
@NamedQuery(name="DTOInternTextExport.primaryKeyQuery", query="SELECT e FROM DTOInternTextExport e WHERE e.AnzeigeText = ?1 AND e.DatenartKrz = ?2 AND e.DBFormat = ?3 AND e.Feldname = ?4")
@NamedQuery(name="DTOInternTextExport.all.migration", query="SELECT e FROM DTOInternTextExport e WHERE e.AnzeigeText IS NOT NULL AND e.DatenartKrz IS NOT NULL AND e.DBFormat IS NOT NULL AND e.Feldname IS NOT NULL")
