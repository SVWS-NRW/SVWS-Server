...
public class Tabelle_TimestampsSchuelerLeistungsdaten extends SchemaTabelle {
	...
	public final SchemaTabelleSpalte col_tsFehlStd = add("tsFehlStd", SchemaDatentypen.DATETIME, false)
			.setDatenlaenge(3)
			.setNotNull()
			.setJavaComment("Der Zeitstempel (UTC) der letzten Änderung an den Fehlstunden.");
	...
	public final SchemaTabelleSpalte col_tsuFehlStd = add("tsuFehlStd", SchemaDatentypen.DATETIME, false)
			.setDatenlaenge(3)
			.setNotNull()
			.setJavaComment("Der Zeitstempel (UTC) der letzten Änderung an den unentschuldigten Fehlstunden.");

	/** Die Definition der Tabellenspalte tsuFehlStd */
	public final SchemaTabelleSpalte col_tsLernentw = add("tsLernentw", SchemaDatentypen.DATETIME, false)
			.setDatenlaenge(3)
			.setNotNull()
			.setJavaComment("Der Zeitstempel (UTC) der letzten Änderung an den fachbezogenen Bemerkungen.");
	...
	public final SchemaTabelleSpalte col_tsWarnung = add("tsWarnung", SchemaDatentypen.DATETIME, false)
			.setDatenlaenge(3)
			.setNotNull()
			.setJavaComment("Der Zeitstempel (UTC) der letzten Änderung, ob gemahnt wird.");
	...
	public final SchemaTabelleTrigger trigger_MariaDB_INSERT_TimestampsSchuelerLeistungsdaten_UNTIL_REV67 = addTrigger(
	...
	public final SchemaTabelleTrigger trigger_MariaDB_INSERT_TimestampsSchuelerLeistungsdaten = addTrigger(
			...
			"""
			AFTER INSERT ON SchuelerLeistungsdaten FOR EACH ROW
			INSERT INTO TimestampsSchuelerLeistungsdaten(ID, tsNotenKrz, tsNotenKrzQuartal, tsFehlStd, tsuFehlStd, tsLernentw, tsWarnung) VALUES (NEW.ID, UTC_TIMESTAMP(3), UTC_TIMESTAMP(3), UTC_TIMESTAMP(3), UTC_TIMESTAMP(3), UTC_TIMESTAMP(3), UTC_TIMESTAMP(3));
			""", Schema.tab_SchuelerLeistungsdaten, Schema.tab_TimestampsSchuelerLeistungsdaten)
