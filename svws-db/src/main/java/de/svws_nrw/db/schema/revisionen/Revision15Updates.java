...
public final class Revision15Updates extends SchemaRevisionUpdateSQL {
	...
	public boolean runLast(final DBEntityManager conn, final Logger logger) {
		...
		if (Integer.MIN_VALUE == conn.transactionNativeUpdateAndFlush("UPDATE K_Vermerkart SET Bezeichnung = 'Bemerkungen (vor Migration)' WHERE Bezeichnung = 'Bemerkungen'")) {
		...
		if (Integer.MIN_VALUE == conn.transactionNativeUpdateAndFlush("INSERT INTO K_Vermerkart(ID, Bezeichnung, Sortierung, Sichtbar, Aenderbar) VALUES (%d, 'Bemerkungen', 1, '+', '+')".formatted(idVermerkart))) {
		...
		final String sql = "INSERT INTO SchuelerVermerke (ID, Schueler_ID, VermerkArt_ID, Datum, Bemerkung, AngelegtVon, GeaendertVon) "
				+ "SELECT ROW_NUMBER() OVER () + (" + idVermerk + "-1) AS ID, Schueler.ID AS Schueler_ID, " + idVermerkart + " AS VermerkArt_ID, now() AS Datum,"
