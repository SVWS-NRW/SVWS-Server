 ...
 */
public class SVWSKonfiguration {
	...
	private HashSet<String> schemataLocked= new HashSet<>();
	 ...
	 */
	public static SVWSKonfiguration getFrom(String path) {
	 ...
	 */
	private static Path findKonfiguration(String path, String filename) throws IOException {
		...
		try {
			ClassLoader classLoader = SVWSKonfiguration.class.getClassLoader();
	 ...
	 */
	private static void readXML(String path, String filename) {
		try {
			Path p = findKonfiguration(path, filename);
		    ...
		    for (int i = 0; i < instanceConfig.getDBAnzahl(); i++) {
		    	SVWSKonfigurationSchemaDTO schema = instanceConfig.dto.dbKonfiguration.schemata.get(i);
	 ...
	 */
	private static void readJSON(String path, String filename) {
		try {
			Path p = findKonfiguration(path, filename);
	 ...
	 * 
	 * @param filename   der Name der zu schreibenden JSON-Konfigurationsdatei
	 * 
	 ...
	 * 
	 * @param filename   der Name der zu schreibenden XML-Konfigurationsdatei
	 * 
	 * @throws IOException   erzeugt eine IOException, wenn die Konfiguration nicht geschrieben werden kann 
	 */
	 ...
	 */
	public static SVWSKonfiguration getDefault(String clientPath, String loggingPath, String tempPath, String keystorePath, String keystorePassword, DBDriver dbms,
			String dbLocation, int dbPort, boolean noSchema, String schema, String schemaUser, String schemaPassword) {
		...
		if (!noSchema) {
			...
			SVWSKonfigurationSchemaDTO schemaConfig = new SVWSKonfigurationSchemaDTO();
	...
	private DBConfig getDBConfig(int i) {
		if ((dto == null) || (i < 0) || (i >= getDBAnzahl()))
			return null;
		DBDriver driver = DBDriver.valueOf(dto.dbKonfiguration.dbms);
		SVWSKonfigurationSchemaDTO schema = dto.dbKonfiguration.schemata.get(i);
	 ...
	 * @return die Datenbank-Konfiguration
	 */
	public DBConfig getDBConfig(String schema) {
		if (dto == null)
			return null;
	 ...
	 */
	public DBConfig getRootDBConfig(String username, String password) {
		if (dto == null)
			return null;
		DBDriver driver = DBDriver.valueOf(dto.dbKonfiguration.dbms);
	...
	public String getDefaultSchema() {
		...
		DBConfig schema = dto.dbconfigs.get(dto.dbKonfiguration.defaultschema);
		if (schema != null)
	 ...
	 */
	public synchronized boolean lockSchema(String schemaName) {
	 ...
	 */
	public synchronized boolean unlockSchema(String schemaName) {
		if (schemataLocked.contains(schemaName)) {
	 ...
	 */
	public synchronized boolean isLockedSchema(String schemaName) {
	...
	public List<DBSchemaListeEintrag> getSchemaList() {
    	...
    	return dto.dbconfigs.values().stream()
    			...
    			.filter(s -> s != null).sorted().map(s -> { 
    				DBSchemaListeEintrag result = new DBSchemaListeEintrag();
    				result.name = s;
	 ...
	 */
	private SVWSKonfigurationSchemaDTO getSchemaKonfiguration(String schemaName) {
		if ((dto == null) || (schemaName == null) || ("".equals(schemaName)))
			return null;
		for (SVWSKonfigurationSchemaDTO config : dto.dbKonfiguration.schemata) {
	 ...
	 */
	public boolean createOrUpdateSchema(String schemaName, String userName, String userPassword, boolean userSVWSLogin) {
		if ((dto == null) || (schemaName == null) || ("".equals(schemaName)) ||
	...
	public static PrivateKey getPrivateKey() throws KeyStoreException {
        try {
			...
			Key key = keystore.getKey(config.getTLSKeyAlias(), config.getTLSKeystorePassword().toCharArray());
	...
	public static String getCertificateBase64() throws KeyStoreException {
        try {
	    	SVWSKonfiguration config = SVWSKonfiguration.get();
			KeyStore keystore = getKeystore();
			Certificate cert = keystore.getCertificate(config.getTLSKeyAlias());			
	...
	public static String getPublicKeyBase64() throws KeyStoreException {
    	PublicKey pubKey = SVWSKonfiguration.getPublicKey();
