 ...
 *
 * <p>Die Uhr läuft in der Zeitzone des Servers ({@link ZoneId#systemDefault()}). Ein Report weist damit dieselbe Zeit aus, die auch sonst auf dem
 * Rechner gilt, auf dem der Server betrieben wird. Die Zone wird bei jedem Aufruf neu ermittelt und nicht in einer Konstanten festgehalten, damit
 * eine Umkonfiguration — etwa über {@code -Duser.timezone} — auch dann wirkt, wenn die Klasse bereits geladen ist.</p>
 *
 ...
 * Datum {@link #SNAPSHOT_FIXED_DATE} aktiviert werden, sodass Snapshot-Tests deterministische Zeitstempel erhalten. Der feste Zeitpunkt wird als
 * Tagesbeginn in derselben Zone verankert, in der anschließend gelesen wird. Die Uhr liefert deshalb unabhängig von der Zeitzone des Servers immer
 * denselben Wert — den 29.02.1948 um 00:00 Uhr —, und ein Lauf im Container ergibt dieselben Snapshots wie einer auf dem Entwicklungsrechner.
 * Außerhalb von DEV wird die Variable ignoriert.</p>
 */
public final class ReportingUhr {
	 ...
	 * {@value #ENV_USE_FIXED_DATE} wird eine feste Uhr auf {@link #SNAPSHOT_FIXED_DATE} geliefert, ansonsten die Systemzeit. Beide Uhren tragen die
	 * Zeitzone des Servers, sodass alles, was über {@code java.time} aus ihnen gelesen wird, diese Zone übernimmt.
	 *
