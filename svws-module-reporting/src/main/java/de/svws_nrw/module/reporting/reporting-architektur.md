...
**Die beiden GOSt-Planungs-Repositories arbeiten mit einem Manager.** `ReportingRepositoryGostKlausurplanung` und `ReportingRepositoryGostKursplanung` verwalten je einen Manager pro Reporting-Request und bauen ihn beim ersten Zugriff selbst auf. Die Aufbauangaben lesen sie aus den Reportparametern: die Kursplanung die ID des Blockungsergebnisses, die Klausurplanung die Stufen aus Abiturjahrgang und GOSt-Halbjahr. Liegen dort keine Stufen vor, leitet die Klausurplanung drei Stufen aus dem ausgewählten Schuljahresabschnitt ab. Die daraus abgeleiteten Reporting-Objekte (Klausurtermine, Kursklausuren und Schülerklausuren bzw. Blockungsergebnis, Schienen und Kurse) werden zentral im Repository aufgebaut und gecached.

**Eine Reihenfolge ist dabei nicht einzuhalten.** Jeder Daten-Getter stößt den Aufbau an, und ein Zugriff vor der Context-Erzeugung — etwa aus der `EmailFactory` — liefert dieselben Daten wie jeder andere. Weil dieser Weg die Prüfung des Initializers umgeht, weisen beide Repositories eine fehlende oder unbrauchbare Angabe selbst mit `BAD_REQUEST` ab, bevor sie Daten laden.

**Das Aufbaumuster ist verbindlich** und in den Konventionen (Abschnitt 1) festgehalten.
...
Die Stufen der GOSt-Klausurplanung (kombinierte IDs aus Abiturjahrgang und GOSt-Halbjahr, z. B. 20261 für (2026, EF.2)) sind Nutzlast wie die IDs eines Listenreports. Ihre Prüfung ist auf zwei Stellen verteilt: Der Initializer prüft Form und Wertebereich der übergebenen IDs und weist einen Verstoß mit `BAD_REQUEST` ab. Die Auswahl selbst trifft das Repository beim Aufbau des Klausurplans, denn sie braucht Datenzugriff — dort werden die Stufen ohne Übergabe aus dem Schuljahresabschnitt abgeleitet, gegen die vorhandenen Abiturjahrgänge abgeglichen und ein fehlender Jahrgang ausgelassen und gemeldet. Weil das Repository auch ohne den Initializer erreichbar ist, prüft es dieselben Grenzen ein zweites Mal: Ohne gültiges GOSt-Halbjahr liefen der Ladeweg und die Meldung der Auslassung in eine Zugriffsverletzung.
