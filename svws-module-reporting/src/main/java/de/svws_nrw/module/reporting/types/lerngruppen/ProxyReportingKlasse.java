...
public class ProxyReportingKlasse extends ReportingKlasse {
	 ...
	 */
	public ProxyReportingKlasse(final ReportingContext reportingContext, final KlassenDaten klassenDaten) {
		super(klassenDaten.id,
				...
				new ArrayList<>(reportingContext.repositoryLehrer().lehrer(klassenDaten.klassenLeitungen.stream().filter(Objects::nonNull).toList(), false)),
				new ArrayList<>(),
		...
		this.schuljahresabschnitt = this.reportingContext.repositorySchule().schuljahresabschnitt(klassenDaten.idSchuljahresabschnitt);
	...
	public ReportingJahrgang jahrgang() {
		if ((super.jahrgang() == null) && (super.idJahrgang() != null) && (super.idJahrgang() >= 0)) {
			final JahrgangsDaten jahrgangsDaten = this.reportingContext.repositoryKataloge().jahrgang(super.idJahrgang());
			if (jahrgangsDaten != null) {
				super.jahrgang = new ProxyReportingJahrgang(this.reportingContext, jahrgangsDaten, this.schuljahresabschnitt);
			}
		}
	...
	public ReportingKlasse vorgaengerklasse() {
		if ((super.vorgaengerklasse() == null) && (super.idVorgaengerklasse() != null) && (super.idVorgaengerklasse() >= 0)) {
			super.vorgaengerklasse = this.reportingContext.repositoryLerngruppen().klasse(super.idVorgaengerklasse());
		}
