 ...
 */
public record ReportingProblemSchluessel(Class<?> objektart, Long id) {
	...
	public static ReportingProblemSchluessel fuer(final Class<?> objektart, final long id) {
		return new ReportingProblemSchluessel(objektart, id);
	}
	...
	public static ReportingProblemSchluessel fuer(final Class<?> objektart) {
		return new ReportingProblemSchluessel(objektart, null);
	}

	/**
	 * Gibt den Schlüssel in einer für das Log lesbaren Form zurück.
	 *
	 * @return Die Objektart mit der ID des betroffenen Datensatzes oder mit dem Vermerk, dass das Problem den gesamten Aufruf betrifft.
	 */
