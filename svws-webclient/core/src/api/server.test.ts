...
import type { LehrerListeEintrag, List } from "../index";
import { ApiServer, BetriebListeEintrag, DBSchemaListeEintrag, Erzieherart, ErzieherListeEintrag, FachDaten, GostFach } from "../index";

const username = "Admin";
const password = "";
const schema = "svwsdb";
const url = "https://www.svws-nrw.de";
...
describe("Server", () => {
	...
	test.skip("patchErzieherStammdaten", async () => {
		const res = await server.patchErzieherStammdaten(
			{ hausnummer: "443" },
			schema,
			1
		);
	...
	test.skip("getKatalogHaltestellen", async () => {
		const res = await server.getHaltestellen(schema);
		expect(res).matchSnapshot();
	});
