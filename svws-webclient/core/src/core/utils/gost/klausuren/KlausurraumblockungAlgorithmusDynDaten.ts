...
import { GostKlausurraumblockungKonfiguration } from '../../../../core/data/gost/klausuren/GostKlausurraumblockungKonfiguration';
...
export class KlausurraumblockungAlgorithmusDynDaten extends JavaObject {
	...
	/**
	 * Lädt den gespeicherten Zustand die {@link GostKlausurraumRich#idsSchuelerklausurtermine}-Liste.
	...
	aktionLadeGespeichertenZustandInDieConfig(): void {
		for (let r: number = 0; r < this._raumAnzahl; r++) {
			...
			raum.idsSchuelerklausurtermine.clear();
			for (let kg: number = 0; kg < this._klausurGruppenAnzahl; kg++) {
				...
				for (const klausur of this._klausurGruppen.get(kg)) {
					raum.idsSchuelerklausurtermine.add(klausur.id);
				}
			}
		}
	...
	transpilerCanonicalName(): string {
		return 'de.svws_nrw.core.utils.gost.klausuren.KlausurraumblockungAlgorithmusDynDaten';
	}

	isTranspiledInstanceOf(name: string): boolean {
		return ['de.svws_nrw.core.utils.gost.klausuren.KlausurraumblockungAlgorithmusDynDaten'].includes(name);
	}

	public static readonly class = new Class<KlausurraumblockungAlgorithmusDynDaten>('de.svws_nrw.core.utils.gost.klausuren.KlausurraumblockungAlgorithmusDynDaten');

}

export function cast_de_svws_nrw_core_utils_gost_klausuren_KlausurraumblockungAlgorithmusDynDaten(obj: unknown): KlausurraumblockungAlgorithmusDynDaten {
