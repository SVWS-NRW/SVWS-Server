...
export class SchildReportingGostLaufbahnplanungSchuelerSummen extends JavaObject {
	...
	isTranspiledInstanceOf(name : string): boolean {
		return ['de.svws_nrw.core.data.schild3.reporting.SchildReportingGostLaufbahnplanungSchuelerSummen'].includes(name);
	}

	public static transpilerFromJSON(json : string): SchildReportingGostLaufbahnplanungSchuelerSummen {
		const obj = JSON.parse(json);
		const result = new SchildReportingGostLaufbahnplanungSchuelerSummen();
	...
	public static transpilerToJSON(obj : SchildReportingGostLaufbahnplanungSchuelerSummen) : string {
		let result = '{';
		result += '"schuelerID" : ' + obj.schuelerID + ',';
		result += '"kursanzahlEF1" : ' + obj.kursanzahlEF1 + ',';
		result += '"kursanzahlEF2" : ' + obj.kursanzahlEF2 + ',';
	...
	public static transpilerToJSONPatch(obj : Partial<SchildReportingGostLaufbahnplanungSchuelerSummen>) : string {
		let result = '{';
		if (typeof obj.schuelerID !== "undefined") {
			result += '"schuelerID" : ' + obj.schuelerID + ',';
		}
...
export function cast_de_svws_nrw_core_data_schild3_reporting_SchildReportingGostLaufbahnplanungSchuelerSummen(obj : unknown) : SchildReportingGostLaufbahnplanungSchuelerSummen {
	return obj as SchildReportingGostLaufbahnplanungSchuelerSummen;
}
