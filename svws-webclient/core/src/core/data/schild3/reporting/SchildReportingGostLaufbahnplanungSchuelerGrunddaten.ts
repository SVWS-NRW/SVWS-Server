...
export class SchildReportingGostLaufbahnplanungSchuelerGrunddaten extends JavaObject {
	...
	isTranspiledInstanceOf(name : string): boolean {
		return ['de.svws_nrw.core.data.schild3.reporting.SchildReportingGostLaufbahnplanungSchuelerGrunddaten'].includes(name);
	}

	public static transpilerFromJSON(json : string): SchildReportingGostLaufbahnplanungSchuelerGrunddaten {
		const obj = JSON.parse(json);
		const result = new SchildReportingGostLaufbahnplanungSchuelerGrunddaten();
	...
	public static transpilerToJSON(obj : SchildReportingGostLaufbahnplanungSchuelerGrunddaten) : string {
		let result = '{';
		result += '"schuelerID" : ' + obj.schuelerID + ',';
		result += '"abiturjahr" : ' + obj.abiturjahr + ',';
	...
	public static transpilerToJSONPatch(obj : Partial<SchildReportingGostLaufbahnplanungSchuelerGrunddaten>) : string {
		let result = '{';
		if (typeof obj.schuelerID !== "undefined") {
			result += '"schuelerID" : ' + obj.schuelerID + ',';
		}
...
export function cast_de_svws_nrw_core_data_schild3_reporting_SchildReportingGostLaufbahnplanungSchuelerGrunddaten(obj : unknown) : SchildReportingGostLaufbahnplanungSchuelerGrunddaten {
	return obj as SchildReportingGostLaufbahnplanungSchuelerGrunddaten;
}
