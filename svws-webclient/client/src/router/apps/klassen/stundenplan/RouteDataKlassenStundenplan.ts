...
export class RouteDataKlassenStundenplan extends RouteData<RouteStateKlassenDataStundenplan> {
	...
	get ganzerStundenplan(): boolean {
		return configStateImpl.config.getValue("klasse.stundenplan.ganzerStundenplan") === 'true';
	}

	setGanzerStundenplan = async (value: boolean) => {
		await configStateImpl.config.setValue("klasse.stundenplan.ganzerStundenplan", value ? "true" : "false");
	};
