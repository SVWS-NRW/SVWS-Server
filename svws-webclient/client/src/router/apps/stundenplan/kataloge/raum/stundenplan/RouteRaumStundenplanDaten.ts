...
import { routeKatalogRaeume } from "~/router/apps/stundenplan/kataloge/raum/RouteKatalogRaeume";
import { routeRaumStundenplan, type RouteRaumStundenplan } from "~/router/apps/stundenplan/kataloge/raum/stundenplan/RouteRaumStundenplan";

import { StundenplanRaum } from "@comp";
import type { StundenplanRaumProps } from "@comp";
import { routeApp } from "~/router/apps/RouteApp";

export class RouteRaumStundenplanDaten extends RouteNode<any, RouteRaumStundenplan> {

	public constructor() {
		super(Schulform.values(), [ BenutzerKompetenz.STUNDENPLAN_ALLGEMEIN_ANSEHEN ], "stundenplan.kataloge.raeume.stundenplan.daten", ":idStundenplan(\\d+)?/:wochentyp(\\d+)?/:kw(\\d+\\.\\d+)?", StundenplanRaum);
