...
import { routeKatalogRaeume, type RouteKatalogRaeume } from "~/router/apps/stundenplan/kataloge/raum/RouteKatalogRaeume";
import { routeRaumStundenplanDaten } from "~/router/apps/stundenplan/kataloge/raum/stundenplan/RouteRaumStundenplanDaten";
import { RouteDataRaumStundenplan } from "~/router/apps/stundenplan/kataloge/raum/stundenplan/RouteDataRaumStundenplan";
...
const SRaumStundenplan = () => import("~/components/stundenplan/kataloge/raeume/stundenplan/SRaumStundenplan.vue");

export class RouteRaumStundenplan extends RouteNode<RouteDataRaumStundenplan, RouteKatalogRaeume> {

	public constructor() {
		super(Schulform.values(), [ BenutzerKompetenz.STUNDENPLAN_ALLGEMEIN_ANSEHEN ], "stundenplan.kataloge.raeume.stundenplan", "stundenplan", SRaumStundenplan, new RouteDataRaumStundenplan());
		...
		if (to.name === this.name) {
			...
			if (routeRaumStundenplan.data.mapStundenplaene.size !== 0) {
				...
				if (stundenplan !== undefined)
					return routeRaumStundenplanDaten.getRoute(idRaum, stundenplan[0], 0);
			}
		}
	}
