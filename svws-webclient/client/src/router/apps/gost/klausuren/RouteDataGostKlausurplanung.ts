...
import { ListUtils, GostKlausurtermin, ArrayList, StundenplanManager, GostFaecherManager, GostHalbjahr, GostKlausurplanManager, DeveloperNotificationException, GostSchuelerklausurtermin, GostKlausurenAlleKlausurdaten, GostKlausurenHalbjahresdaten, GostKursklausur } from "@core";
...
export class RouteDataGostKlausurplanung extends RouteData<RouteStateGostKlausurplanung> {
	...
	gotoKalenderdatum = async (datum: string | undefined, termin: GostKlausurtermin | undefined) => {
		if (termin !== undefined) {
			if (datum === undefined) {
				await RouteManager.doRoute(routeGostKlausurplanungKalender.getRoute({ abiturjahr: termin.abiturjahrgang, halbjahr: termin.halbjahr, datum: (termin.datum === null ? -1 : termin.datum.replaceAll("-", "")), idtermin: termin.id }));
			} else {
				await RouteManager.doRoute(routeGostKlausurplanungKalender.getRoute({ abiturjahr: termin.abiturjahrgang, halbjahr: termin.halbjahr, datum: datum.replaceAll("-", ""), idtermin: termin.id }));
			}
		...
		try {
			...
			if (klausur instanceof GostKursklausur) {
				...
				if (result.kursklausurPatched === null) {
					throw new Error("Die gepatchte Kursklausur fehlt in der API-Response.");
				}
