...
import type { Abteilung, KlasseListItem, List, Schuljahresabschnitt, SimpleOperationResponse } from "@core";
...
export class RouteDataAbteilungen extends RouteDataAuswahl<AbteilungenListeManager, RouteStateAuswahlInterface<AbteilungenListeManager>> {
	...
	protected async createManager(idSchuljahresabschnitt: number): Promise<Partial<RouteStateAuswahlInterface<AbteilungenListeManager>>> {
		...
		const klassenAktAbschnitt = await api.server.getKlassenListItemsBySchuljahresabschnitt(api.schema, idSchuljahresabschnitt);

		const abteilungenFolgeAbschnitt = new ArrayList<Abteilung>();
		const klassenFolgeAbschnitt = new ArrayList<KlasseListItem>();
		...
		if (this.istSchuleAbschnittUndHatFolgeAbschnitt(schuljahresabschnitt)) {
			...
			klassenFolgeAbschnitt.addAll(await api.server.getKlassenListItemsBySchuljahresabschnitt(api.schema, schuljahresabschnitt.idFolgeAbschnitt!));
		}
