...
<script setup lang="ts">
	...
	const rechtsverhaeltnis = computed<LehrerRechtsverhaeltnis | undefined>({
		get(): LehrerRechtsverhaeltnis | undefined {
			return LehrerRechtsverhaeltnis.values().find(r => r.daten(schuljahr.value)?.kuerzel === personalabschnittsdaten.value?.rechtsverhaeltnis);
		},
		set(val: LehrerRechtsverhaeltnis | undefined) {
			if (personalabschnittsdaten.value !== null)
				void props.patchAbschnittsdaten({ rechtsverhaeltnis: val?.daten(schuljahr.value)?.kuerzel }, personalabschnittsdaten.value.id);
		}
	});

	const beschaeftigungsart = computed<LehrerBeschaeftigungsart | undefined>({
		get(): LehrerBeschaeftigungsart | undefined {
			return LehrerBeschaeftigungsart.values().find(r => r.daten(schuljahr.value)?.kuerzel === personalabschnittsdaten.value?.beschaeftigungsart);
		},
		set(val: LehrerBeschaeftigungsart | undefined) {
			if (personalabschnittsdaten.value !== null)
				void props.patchAbschnittsdaten({ beschaeftigungsart: val?.daten(schuljahr.value)?.kuerzel }, personalabschnittsdaten.value.id);
		}
	});

	const einsatzstatus = computed<LehrerEinsatzstatus | undefined>({
		get(): LehrerEinsatzstatus | undefined {
			return LehrerEinsatzstatus.values().find(r => r.daten(schuljahr.value)?.kuerzel === personalabschnittsdaten.value?.einsatzstatus);
		},
		set(val: LehrerEinsatzstatus | undefined) {
			if (personalabschnittsdaten.value !== null)
				void props.patchAbschnittsdaten({ einsatzstatus: val?.daten(schuljahr.value)?.kuerzel }, personalabschnittsdaten.value.id);
		}
	});
