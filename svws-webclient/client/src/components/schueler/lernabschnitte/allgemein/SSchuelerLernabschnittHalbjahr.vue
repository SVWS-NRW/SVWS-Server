<template>
	<svws-ui-content-card title="Allgemeine Angaben">
		<svws-ui-input-wrapper :grid="2">
			...
			<svws-ui-spacing />
			<svws-ui-input-wrapper :grid="2">
				<svws-ui-select title="Förderschwerpunkt" :items="manager().foerderschwerpunktGetMenge()" :item-text="i => i.text" autocomplete
					v-model="foerderschwerpunkt" />
				<svws-ui-select title="Weiterer Förderschwerpunkt" :items="manager().foerderschwerpunktGetMenge()" :item-text="i => i.text" autocomplete
			...
			<svws-ui-input-wrapper>
				...
				<div v-if="manager().lernabschnittGet().nachpruefungen !== null">
					...
					<div v-for="fach in manager().lernabschnittGet().nachpruefungen!.moegliche" :key="fach">
						{{ fach }}
					</div>
					<div>Nachprüfungen</div>
					<div v-for="pruefung in manager().lernabschnittGet().nachpruefungen!.pruefungen" :key="pruefung.fachID">
...
<script setup lang="ts">
	...
	import type { FoerderschwerpunktEintrag, LehrerListeEintrag, List, OrganisationsformKatalogEintrag, SchuelerLernabschnittManager, SchuelerLernabschnittsdaten, SchuleStammdaten} from "@core";
