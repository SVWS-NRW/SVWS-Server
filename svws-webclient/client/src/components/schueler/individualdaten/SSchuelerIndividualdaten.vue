	...
	<div class="page--content">
		<svws-ui-content-card title="Allgemein">
			<svws-ui-input-wrapper :grid="2">
				...
				<svws-ui-spacing />
				<svws-ui-select title="Geschlecht" :disabled="!hatKompetenzUpdate" v-model="geschlecht" :items="Geschlecht.values()" statistics :item-text="(i: Geschlecht)=>i.text" />
				<svws-ui-text-input placeholder="Geburtsdatum" :disabled="!hatKompetenzUpdate" :model-value="data.geburtsdatum" @change="geburtsdatum => geburtsdatum && patch({geburtsdatum})" type="date" :valid="istGeburtsdatumGueltig" required statistics />
				<svws-ui-text-input placeholder="Geburtsort" :disabled="!hatKompetenzUpdate" :model-value="data.geburtsort" @change="geburtsort => patch({ geburtsort })" type="text" />
				<svws-ui-text-input placeholder="Geburtsname" :disabled="!hatKompetenzUpdate" :model-value="data.geburtsname" @change="geburtsname => patch({ geburtsname })" type="text" />
		...
		<svws-ui-content-card title="Wohnort und Kontaktdaten" v-if="hatKompetenzAnsehen">
			<svws-ui-input-wrapper :grid="2">
				...
				<svws-ui-select title="Wohnort" :disabled="!hatKompetenzUpdate" v-model="wohnortID" :items="mapOrte" :item-filter="orte_filter" :item-sort="orte_sort" :item-text="(i: OrtKatalogEintrag) => `${i.plz} ${i.ortsname}`" autocomplete statistics />
				<svws-ui-select title="Ortsteil" :disabled="!hatKompetenzUpdate" v-model="ortsteilID" :items="ortsteile" :item-text="(i: OrtsteilKatalogEintrag) => i.ortsteil ?? ''" :item-sort="ortsteilSort" :item-filter="ortsteilFilter" removable />
				<svws-ui-spacing />
				...
				<svws-ui-text-input placeholder="Mobil oder Fax" :disabled="!hatKompetenzUpdate" :model-value="data.telefonMobil" @change="telefonMobil => patch({ telefonMobil })" type="tel" />
				<svws-ui-text-input placeholder="Private E-Mail-Adresse" :disabled="!hatKompetenzUpdate" :model-value="data.emailPrivat" @change="emailPrivat => patch({ emailPrivat })" type="email" verify-email />
				<svws-ui-text-input placeholder="Schulische E-Mail-Adresse" :disabled="!hatKompetenzUpdate" :model-value="data.emailSchule" @change="emailSchule => patch({ emailSchule })" type="email" verify-email />
			</svws-ui-input-wrapper>
		</svws-ui-content-card>
		<svws-ui-content-card title="Staatsangehörigkeit und Konfession" v-if="hatKompetenzAnsehen">
			<svws-ui-input-wrapper :grid="2">
				...
				<svws-ui-text-input placeholder="Abmeldung vom Religionsunterricht" :disabled="!hatKompetenzUpdate" :model-value="data.religionabmeldung" @change="religionabmeldung => patch({religionabmeldung})" type="date" />
				<svws-ui-text-input placeholder="Wiederanmeldung" :disabled="!hatKompetenzUpdate" :model-value="data.religionanmeldung" @change="religionanmeldung => patch({religionanmeldung})" type="date" />
			</svws-ui-input-wrapper>
		</svws-ui-content-card>
		<svws-ui-content-card title="Migrationshintergrund" v-if="hatKompetenzAnsehen">
			<template #actions>
				<svws-ui-checkbox :disabled="!hatKompetenzUpdate" class="mt-3 xl:mt-0" :model-value="hatMigrationshintergrund" @update:model-value="hatMigrationshintergrund => patch({hatMigrationshintergrund})">
		...
		<svws-ui-content-card title="Statusdaten" v-if="hatKompetenzAnsehen">
			...
			<svws-ui-input-wrapper :grid="2">
				<svws-ui-select title="Status" :disabled="!hatKompetenzUpdate" :model-value="SchuelerStatus.fromID(data.status)" @update:model-value="status => patch({ status: status?.id })" :items="SchuelerStatus.values()" :item-text="i => i.bezeichnung" statistics />
				<svws-ui-select v-if="schuelerListeManager().daten().status === SchuelerStatus.EXTERN.id" :disabled="!hatKompetenzUpdate"
					title="Stammschule" v-model="inputStammschule" :items="mapSchulen.values()" :item-text="i => i.kuerzel ?? i.schulnummer" removable />
				<div v-else />
				<svws-ui-select title="Fahrschüler" :disabled="!hatKompetenzUpdate" v-model="inputFahrschuelerArtID" :items="mapFahrschuelerarten" :item-text="i => i.text ?? ''" removable />
				<svws-ui-select title="Haltestelle" :disabled="!hatKompetenzUpdate" v-model="inputHaltestelleID" :items="mapHaltestellen" :item-text="i => i.text ?? ''" removable />
				<svws-ui-text-input placeholder="Anmeldedatum" :disabled="!hatKompetenzUpdate" :model-value="data.anmeldedatum" @change="d => patch({ anmeldedatum : d ?? null })" type="date" removable />
				<svws-ui-text-input placeholder="Aufnahmedatum" :disabled="!hatKompetenzUpdate" :model-value="data.aufnahmedatum" @change="aufnahmedatum => patch({aufnahmedatum})" type="date" statistics />
				<svws-ui-spacing />
				<svws-ui-input-wrapper :grid="2" class="input-wrapper--checkboxes">
					<svws-ui-checkbox :disabled="!hatKompetenzUpdate" :model-value="data.istVolljaehrig === true" @update:model-value="istVolljaehrig => patch({ istVolljaehrig })"> Volljährig </svws-ui-checkbox>
					<svws-ui-checkbox :disabled="!hatKompetenzUpdate" :model-value="data.keineAuskunftAnDritte" @update:model-value="keineAuskunftAnDritte => patch({ keineAuskunftAnDritte })"> Keine Auskunft an Dritte </svws-ui-checkbox>
					<svws-ui-checkbox :disabled="!hatKompetenzUpdate" :model-value="data.istSchulpflichtErfuellt === true" readonly> Schulpflicht erfüllt </svws-ui-checkbox>
					<svws-ui-checkbox :disabled="!hatKompetenzUpdate" :model-value="data.istBerufsschulpflichtErfuellt === true" @update:model-value="istBerufsschulpflichtErfuellt => patch({ istBerufsschulpflichtErfuellt })"> Schulpflicht SII erfüllt </svws-ui-checkbox>
					<svws-ui-checkbox :disabled="!hatKompetenzUpdate" :model-value="data.hatMasernimpfnachweis" @update:model-value="hatMasernimpfnachweis => patch({ hatMasernimpfnachweis })"> Masern Impfnachweis </svws-ui-checkbox>
					<svws-ui-checkbox :disabled="!hatKompetenzUpdate" :model-value="data.erhaeltSchuelerBAFOEG" @update:model-value="erhaeltSchuelerBAFOEG => patch({ erhaeltSchuelerBAFOEG })">BAFöG</svws-ui-checkbox>
				</svws-ui-input-wrapper>
			</svws-ui-input-wrapper>
		</svws-ui-content-card>
	</div>
...
<script setup lang="ts">
	...
	const hatKompetenzLoeschen = computed<boolean>(() => props.benutzerKompetenzen.has(BenutzerKompetenz.SCHUELER_INDIVIDUALDATEN_LOESCHEN));
