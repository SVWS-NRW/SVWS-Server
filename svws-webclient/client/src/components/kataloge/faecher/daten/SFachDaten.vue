	...
	<div class="page--content">
		<svws-ui-content-card title="Allgemein">
			<svws-ui-input-wrapper :grid="2">
				...
				<svws-ui-select title="Statistik-Fach" :model-value="ZulaessigesFach.getByKuerzelASD(fachListeManager().daten().kuerzelStatistik)"
				...
				<svws-ui-text-input placeholder="Fachgruppe" :model-value="ZulaessigesFach.getByKuerzelASD(fachListeManager().daten().kuerzelStatistik)?.getFachgruppe()?.daten.bezeichnung ?? ''" disabled />
				<svws-ui-select title="Bilinguale Sachfachsprache" :model-value="BilingualeSprache.getByKuerzel(fachListeManager().daten().bilingualeSprache)"
		...
		<svws-ui-content-card title="Zeugnis">
			<svws-ui-input-wrapper :grid="2">
				...
				<svws-ui-text-input placeholder="Bezeichnung (Zeugnis)" :model-value="fachListeManager().daten().bezeichnungZeugnis" @change="value => patch({ bezeichnungZeugnis: value })" />
				<svws-ui-text-input placeholder="Bezeichnung (Überweisungszeugnis)" :model-value="fachListeManager().daten().bezeichnungUeberweisungszeugnis" @change="value => patch({ bezeichnungUeberweisungszeugnis: value })" />
			</svws-ui-input-wrapper>
		</svws-ui-content-card>
		<svws-ui-content-card title="Sonstiges">
			<svws-ui-input-wrapper :grid="1">
				...
				<svws-ui-checkbox v-if="fachListeManager().schulform().daten.hatGymOb" :model-value="fachListeManager().daten().istOberstufenFach" @update:model-value="value => patch({ istOberstufenFach: value === true ? true : false })">
					Fach der Oberstufe
				</svws-ui-checkbox>
				<svws-ui-checkbox v-if="fachListeManager().schulform().daten.hatGymOb" :model-value="fachListeManager().daten().istPruefungsordnungsRelevant" @update:model-value="value => patch({ istPruefungsordnungsRelevant: value === true ? true : false })">
				...
				<svws-ui-input-number placeholder="maximale Zeichenanzahl in Fachbemerkungen" :model-value="data().maxZeichenInFachbemerkungen === JavaInteger.MAX_VALUE ? null : data().maxZeichenInFachbemerkungen" @blur="z => patch({ maxZeichenInFachbemerkungen: z ?? JavaInteger.MAX_VALUE-1 })" :min="0" :max="JavaInteger.MAX_VALUE-1" :valid="i => (i === null) || (i > 0) && (i < JavaInteger.MAX_VALUE)" />
			</svws-ui-input-wrapper>
		</svws-ui-content-card>
	</div>
</template>
