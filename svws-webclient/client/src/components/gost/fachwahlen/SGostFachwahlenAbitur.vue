				...
				<template v-if="hatAbiFachwahl(fws)">
					...
					<div v-if="aktuell?.id === fws.id" role="row" class="svws-ui-tr text-ui">
						<div role="cell" class="flex flex-col svws-ui-td mb-5 leading-tight" v-for="abifach in abifaecher" :key="abifach.id">
							<div v-for="schueler in getSchuelerListe(fws.id, abifach)" :key="schueler.id" class="flex justify-start w-full gap-1 py-0.5 px-4 -mx-1 -mt-0.5 hover:bg-ui-75 rounded-sm cursor-pointer" role="link" @click="gotoLaufbahnplanung(schueler.id)">
