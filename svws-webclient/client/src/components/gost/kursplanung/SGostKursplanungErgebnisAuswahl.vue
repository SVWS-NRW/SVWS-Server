<template>
	<template v-if="!getErgebnisse().isEmpty()">
		<svws-ui-table clickable :clicked="auswahlErgebnis" @update:clicked="setAuswahlErgebnis" v-model="selected_ergebnisse" :selectable="getErgebnisse().size() > 1" class="z-20 relative"
			:columns="[{ key: 'bewertung', label: ' ' }, { key: 'id', label: 'ID', fixedWidth: 6, align: 'right'}, {key: 'aktiviert', label: ' ', fixedWidth: 2, align: 'right'}]" :items="getErgebnisse()" :count="getErgebnisse().size() > 1">
			...
			<template #cell(bewertung)="{ rowData: row }">
				<div class="inline-flex flex-wrap w-full gap-x-1 gap-y-2.5">
					<span class="flex gap-1 items-center ml-0.5">
						<span class="svws-ui-badge min-w-[3.3rem] text-center justify-center" :title="`${getDatenmanager().ergebnisGetBewertung1Wert(row.id)} Regelverletzungen`" :style="{'background-color': color1(row)}">{{ getDatenmanager().ergebnisGetBewertung1Wert(row.id) }}</span>
						<span class="svws-ui-badge min-w-[3.3rem] text-center justify-center" :title="`${getDatenmanager().ergebnisGetBewertung2Wert(row.id)} Wahlkonflikte`" :style="{'background-color': color2(row)}">{{ getDatenmanager().ergebnisGetBewertung2Wert(row.id) }}</span>
						<span class="svws-ui-badge min-w-[3.3rem] text-center justify-center" :title="`Maximale Kursdifferenz: ${getDatenmanager().ergebnisGetBewertung1Wert(row.id)}`" :style="{'background-color': color3(row)}">{{ getDatenmanager().ergebnisGetBewertung3Wert(row.id) }}</span>
						<span class="svws-ui-badge min-w-[3.3rem] text-center justify-center" :title="`${getDatenmanager().ergebnisGetBewertung2Wert(row.id)} Fächer parallel`" :style="{'background-color': color4(row)}">{{ getDatenmanager().ergebnisGetBewertung4Wert(row.id) }}</span>
					</span>
				</div>
			</template>
			<template #cell(aktiviert)="{rowData: row}">
				<svws-ui-tooltip v-if="row.istVorlage">
					<i-ri-checkbox-circle-fill class="text-svws text-headline-md relative -my-0.5" />
					<template #content>
			...
			<template #actions>
				<template v-if="selected_ergebnisse.length === 0">
					<svws-ui-button type="transparent" v-if="selected_ergebnisse.length === 0" @click.stop="derive_blockung" :disabled="apiStatus.pending || selected_ergebnisse.length > 0" title="Eine neue Blockung auf Grundlage dieses Ergebnisses erstellen."> Ergebnis ableiten </svws-ui-button>
					<svws-ui-button type="icon" class="cursor-pointer" @click.stop="remove_ergebnis" :disabled="apiStatus.pending || getErgebnisse().size() <= 1" title="Ergebnis löschen">
						<i-ri-delete-bin-line class="-mx-0.5" />
					</svws-ui-button>
				</template>
				<svws-ui-button v-else @click="remove_ergebnisse" type="transparent" :disabled="getErgebnisse().size() === 0 || selected_ergebnisse.length >= getErgebnisse().size() || selected_ergebnisse.length === 0" class="text-error">
