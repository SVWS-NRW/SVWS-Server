						...
						<slot name="datum">
							...
							<template v-else>
								<span class="opacity-50 inline-flex items-center gap-1">
									...
									<svws-ui-button v-if="!hideButtonRaeumePlanen" :disabled="!hatKompetenzUpdate" type="transparent" @click="gotoRaumzeitTermin(termin.abiturjahrgang, GostHalbjahr.fromIDorException(termin.halbjahr), termin.id);$event.stopPropagation()" title="Räume planen" size="small"><span class="icon i-ri-link" /> Räume planen</svws-ui-button>
								</span>
							</template>
						</slot>
					</div>
								...
								<div v-for="klausur in kursklausuren()"
									...
									<div class="svws-ui-td" role="cell">
										<svws-ui-tooltip :hover="false" :indicator="false" :keep-open autosize>
											<template #content>
												<s-gost-klausurplanung-kursliste :k-man :kursklausur="klausur" :termin :patch-klausur :create-schuelerklausurtermin @modal="keepOpen = $event" />
											</template>
