<template>
	<div>
		...
		<table class="table-auto min-w-full border -mx-3 border-ui-contrast-25" v-if="kMan().raumGetMengeByTermin(termin).size() > 0">
			<thead class="border-b border-ui-contrast-25 text-left text-button">
				<tr>
					<th class="px-3 py-1.5 border-r border-ui-contrast-25" style="width:8%">Raum</th>
					<th colspan="3" class="px-3 py-1.5 border-r border-ui-contrast-25" style="width:45%">Kursinfos</th>
					<!--<th class="border-r" style="width:15%">Aufsichten</th>-->
					<th class="px-3 py-1.5">Bemerkungen</th>
				</tr>
			</thead>
			<tbody>
				<tr v-for="(raum, id) in kMan().raumGetMengeByTermin(termin)" :key="raum.id" class="border-b border-ui-contrast-25">
					<td v-if="raum.idStundenplanRaum !== null" class="border-r text-center border-ui-contrast-25">
						{{ kMan().stundenplanraumGetByKlausurraum(raum).kuerzel }}
					</td>
					<td v-else class="border-r text-ui-danger text-center border-ui-contrast-25">
						N.N.
					</td>
					<template v-if="kMan().kursklausurGetMengeByRaum(raum).size() > 0">
						<td class="border-ui-contrast-25 border-r" :colspan="kursInfoColspan(raum)">
							<table class="w-full">
						...
						<td class="border-r border-ui-contrast-25 text-center" v-if="kMan().getGemeinsamerKursklausurstartByKlausurraum(raum) !== null">{{ DateUtils.getStringOfUhrzeitFromMinuten(kMan().getGemeinsamerKursklausurstartByKlausurraum(raum)!) }} Uhr</td>
						<td class="border-r border-ui-contrast-25 text-center" v-if="kMan().getGemeinsameKursklausurdauerByKlausurraum(raum) !== null">{{ kMan().getGemeinsameKursklausurdauerByKlausurraum(raum) }} Min.</td>
					...
					<td v-else colspan="3" class="border-ui-contrast-25 border-r text-center text-ui-danger p-2">
