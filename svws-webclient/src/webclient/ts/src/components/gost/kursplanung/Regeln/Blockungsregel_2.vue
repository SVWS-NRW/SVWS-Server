<template>
	<BlockungsregelBase v-model="regel" @update:model-value="e => emit('update:modelValue', e)" :regel-typ="regel_typ" :regeln="regeln"
		...
		<template #beschreibung="{ regel: r }">
			{{ getKursbezeichnung(getKursFromId(kurse, r.parameter.get(0).valueOf()), mapFaecher) }} auf Schiene {{ r.parameter.get(1) }} fixiert
		</template>
		Fixiere
...
<script setup lang="ts">
	...
	import { GostBlockungKurs, GostBlockungRegel, GostBlockungSchiene, GostKursblockungRegelTyp, GostFach } from "@svws-nrw/svws-core-ts";
