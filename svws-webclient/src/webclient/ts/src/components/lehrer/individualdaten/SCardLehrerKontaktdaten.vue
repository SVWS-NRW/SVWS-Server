...
<script setup lang="ts">
	...
	const daten: ComputedRef<LehrerStammdaten> = computed(
		() => daten.value || new LehrerStammdaten()
	);
	...
	const inputEmailDienstlich: WritableComputedRef<string | undefined> =
		computed({
			get(): string | undefined {
				return daten.value.emailDienstlich?.toString();
			},
			set(val) {
				app.stammdaten.patch({ emailDienstlich: val });
			}
		});
</script>
