<template>
	<svws-ui-content-card title="Stundenplan">
		<svws-ui-multi-select v-model="selected_stundenplan" :items="liste" :item-text="(i: StundenplanListeEintrag)=>i.bezeichnung" />
						...
						<tbody>
							<tr v-for="stunde in stundenplan?.getMaxStunde()">
								...
								<td class="text-center border-2 border-collapse" v-for="wochentag in stundenplan?.getMaxWochentag()">
									<table class="w-full">
										<tr>
											<s-unterricht v-for="unterricht in stundenplan?.getUnterrichtByWocheZeitrasterId(wochentyp.valueOf(), stundenplan?.getZeitrasterByWochentagStunde(wochentag, stunde).id, true)" :key="unterricht.idUnterricht" :unterricht="unterricht" />
										</tr>
									</table>
								</td>
							</tr>
