<template>
	<div class="page--content page--content--full min-w-fit gap-x-8 2xl:gap-x-16 relative">
		...
		<div class="flex-grow">
			<s-laufbahnplanung-card-planung v-if="visible" :abiturdaten-manager="abiturdatenManager" :manueller-modus="istManuellerModus"
	...
	async function reset_fachwahlen() {
		for (const fachbelegung of props.abiturdatenManager().getFachbelegungen()) {
			...
			if (fach) {
				...
				await props.setWahl(fach.id, fachwahl);
			}
		}
	}
