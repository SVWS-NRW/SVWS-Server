	...
	<div class="page--content page--content--full min-w-fit gap-x-8 2xl:gap-x-16 relative">
		<Teleport to=".router-tab-bar--subnav-target" v-if="isMounted">
			<svws-ui-sub-nav>
				...
				<svws-ui-button size="small" :type="mod === 'manuell' ? 'error' : 'transparent'" @click="switchModus" :title="mod === 'manuell' ? 'Manueller Modus' : mod === 'normal' ? 'Normaler Modus':'Hochschreibemodus'">
					{{ mod === 'manuell' ? 'Manueller Modus' : mod === 'normal' ? 'Normaler Modus':'Hochschreibemodus' }}
					<template v-if="mod==='manuell'">
		...
		<div class="flex-grow">
			<s-laufbahnplanung-card-planung v-if="visible" :abiturdaten-manager="abiturdatenManager" :manueller-modus="mod==='manuell'"
...
<script setup lang="ts">
	...
	const mod = computed(()=>props.modus());
