...
export class KlausurterminblockungDynDaten extends JavaObject {
	...
	private _schienenAnzahl1 : number = KlausurterminblockungDynDaten.SCHIENEN_MAX_ANZAHL;

	private _schienenAnzahl2 : number = KlausurterminblockungDynDaten.SCHIENEN_MAX_ANZAHL;
	...
	private initialisiereMatrixBevorzugt(pInput : List<GostKursklausur>) : void {
		for (let gostKursklausur1 of pInput) 
			for (let gostKursklausur2 of pInput) 
				if (KlausurterminblockungDynDaten.hatGemeinsameSchiene(gostKursklausur1.kursSchiene, gostKursklausur2.kursSchiene)) {
