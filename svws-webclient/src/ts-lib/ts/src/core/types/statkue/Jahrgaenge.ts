...
export class Jahrgaenge extends JavaObject {
	...
	private static readonly _mapKuerzel : HashMap<String, Jahrgaenge> = new HashMap();

	private static readonly _mapID : HashMap<Number, Jahrgaenge> = new HashMap();
	...
	public static get(schulform : Schulform | null) : List<Jahrgaenge> {
		let result : Vector<Jahrgaenge> = new Vector();
		if (schulform === null) 
			return result;
	...
	isTranspiledInstanceOf(name : string): boolean {
		return ['de.nrw.schule.svws.core.types.statkue.Jahrgaenge'].includes(name);
	}

}

export function cast_de_nrw_schule_svws_core_types_statkue_Jahrgaenge(obj : unknown) : Jahrgaenge {
