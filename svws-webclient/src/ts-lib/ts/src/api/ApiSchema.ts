...
export class ApiSchema extends BaseApi {
	...
	public async exportSQLite(schema : string) : Promise<Blob> {
		let path : string = "/db/{schema}/api/export/sqlite"
	...
	public async isTainted(schema : string) : Promise<Boolean> {
		let path : string = "/db/{schema}/api/isTainted"
				.replace(/{schema\s*(:[^}]+)?}/g, schema);
	...
	public async revision(schema : string) : Promise<Number> {
		let path : string = "/db/{schema}/api/revision"
				.replace(/{schema\s*(:[^}]+)?}/g, schema);
		const result : string = await super.getJSON(path);
		const text = result;
	...
	public async updateSchemaToCurrent(schema : string) : Promise<List<String>> {
		let path : string = "/db/{schema}/api/update"
				.replace(/{schema\s*(:[^}]+)?}/g, schema);
		const result : string = await super.postJSON(path, null);
		const obj = JSON.parse(result);
		let ret = new Vector<String>();
		obj.forEach((elem: any) => { let text : string = JSON.stringify(elem); ret.add(JSON.parse(text).toString()); });
		return ret;
	}
	...
	public async updateSchema(schema : string, revision : number) : Promise<List<String>> {
		let path : string = "/db/{schema}/api/update/{revision : \d+}"
				.replace(/{schema\s*(:[^}]+)?}/g, schema)
