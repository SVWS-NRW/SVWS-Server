<template>
	<Story title="Multiselect 2" id="svws-ui-multi-select2" icon="ri:expand-up-down-fill" auto-props-disabled>
		<Variant title="Default">
			<svws-ui-content-card class="p-5">
				<svws-ui-input-wrapper :grid="2" class="items-start">
					<svws-ui-multi-select-2 v-model="modelValue"
						:label="state.label"
					...
					<svws-ui-multi-select-2 label="Relevant für die Statistik"
						:items="items"
						:item-text="item => item.text"
						v-model="modelValueStatistik"
						statistics />
					<svws-ui-multi-select-2 label="Disabled"
						:items="items"
						:item-text="item => item.text"
						v-model="modelValueDisabled" disabled />
					<svws-ui-spacing />
					<svws-ui-multi-select-2 label="Headless z. B. in der Tabelle"
						:items="items"
						:item-text="item => item.text"
			...
			<template #source>
				{{ `
<svws-ui-multi-select-2 label="Label"
                        :items="items"
                        :item-text="item => item.text"
                        v-model="[]" />
        ` }}
