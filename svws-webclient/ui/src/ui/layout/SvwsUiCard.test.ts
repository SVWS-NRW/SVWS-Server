...
import SvwsUiCard from "./SvwsUiCard.vue";
...
describe("Komponente kann gemounted werden", () => {
	test("HTML wird erzeugt", () => {
		const wrapper = mount(SvwsUiCard);
		expect(wrapper.html()).includes("svws-ui-card");
	});
});
...
describe.concurrent("PropHandhabung läuft korrekt", () => {
	...
	test("Korrekte Eigenschaft bei einem mount mit default Props", () => {
		...
		// Header
		expect(wrapper.find(".svws-ui-card--header").exists()).toBeTruthy();
		expect(wrapper.find(".header--icon").exists()).toBeFalsy();
		expect(wrapper.find(".header--title-wrapper").exists()).toBeFalsy();
		expect(wrapper.find(".header--title").exists()).toBeFalsy();
		expect(wrapper.find(".header--subtitle").exists()).toBeFalsy();
		expect(wrapper.find(".header--right-section").exists()).toBeTruthy();
		expect(wrapper.find(".header--info").exists()).toBeFalsy();
		const collapseIcon = wrapper.findAll(".header--collapse-icon");
		expect(collapseIcon.length === 1).toBeTruthy();
		expect(wrapper.find(".header--right-section").find(".header--collapse-icon")).toBeTruthy();

		// Body
		expect(wrapper.find(".svws-ui-card--body-wrapper").exists()).toBeTruthy();
		expect(wrapper.find(".svws-ui-card--body").exists()).toBeTruthy();
		expect(wrapper.find(".body--content").exists()).toBeTruthy();
		expect(wrapper.find("#svws-ui-card--button-content-left").exists()).toBeFalsy();
		expect(wrapper.find(".content--main").exists()).toBeFalsy();
		expect(wrapper.find("#svws-ui-card--button-content-right").exists()).toBeFalsy();
		expect(wrapper.find("hr").exists()).toBeFalsy();
		expect(wrapper.find(".body--footer").exists()).toBeFalsy();
		expect(wrapper.find(".svws-ui-card--button-footer-left").exists()).toBeFalsy();
		expect(wrapper.find(".footer--main").exists()).toBeFalsy();
		expect(wrapper.find(".svws-ui-card--button-footer-right").exists()).toBeFalsy();
	});

	test("Mit Prop 'compact = true' wird die Komponente kompakter dargestellt", () => {
		const wrapper = mount(SvwsUiCard, { props: { compact: true } });
		expect(wrapper.find(".svws-ui-card").classes()).toContain("compact");
	});

	test("Mit Prop 'compact = false' wird die Komponente nicht kompakt dargestellt", () => {
		const wrapper = mount(SvwsUiCard, { props: { compact: false } });
		expect(wrapper.find(".svws-ui-card").classes()).not.toContain("compact");
	});

	test("Mit Prop 'collapsible = true' wird die Komponente klappbar dargestellt", async () => {
		const setActive = vi.fn();
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true }, global: { mocks: { setActive } } });
		const header = wrapper.find(".svws-ui-card--header");
		expect(wrapper.find(".svws-ui-card--header").element.tagName).toBe("BUTTON");
		expect(wrapper.find(".header--collapse-icon").exists()).toBeTruthy();
	...
	test("Mit Prop 'collapsible = false' wird die Komponente nicht klappbar dargestellt", async () => {
		const setActive = vi.fn();
		const wrapper = mount(SvwsUiCard, { props: { collapsible: false }, global: { mocks: { setActive } } });
		const header = wrapper.find(".svws-ui-card--header");
		expect(header.element.tagName).toBe("DIV");
		expect(wrapper.find(".header--collapse-icon").exists()).toBeFalsy();
		await header.trigger("click");
		expect(setActive).not.toHaveBeenCalled();
	});

	test("Mit Prop 'collapseIconPosition = left' wird die Komponente klappbar mit Collapse-Icon links dargestellt", () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, collapseIconPosition: 'left' } });
		expect(wrapper.find(".svws-ui-card--header").find(".header--collapse-icon").exists()).toBeTruthy();
		expect((wrapper.find(".header--right-section")).find(".header--collapse-icon").exists()).toBeFalsy();
	});

	test("Mit Prop 'collapseIconPosition = right' wird die Komponente klappbar mit Collapse-Icon rechts dargestellt", () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, collapseIconPosition: 'right' } });
		expect(wrapper.find(".svws-ui-card--header").findAll(".header--collapse-icon").length === 1).toBeTruthy();
		expect((wrapper.find(".header--right-section")).find(".header--collapse-icon").exists()).toBeTruthy();
	});

	test("Mit Prop 'collapseIconClosed = i-ri-add-line' wird das Collapse-Icon im geschlossenen Zustand auf ein Plus gesetzt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, collapseIconClosed: 'i-ri-add-line' } });
		expect(wrapper.find(".header--collapse-icon").find("span").classes()).toContain("i-ri-add-line");
	});
	...
	test("Mit Prop 'collapseIconOpened = i-ri-subtract-line' wird das Collapse-Icon im offenen Zustand auf ein Minus gesetzt ", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, collapseIconOpened: 'i-ri-subtract-line', isOpen: true } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		expect(wrapper.find(".header--collapse-icon").find("span").classes()).toContain("i-ri-subtract-line");
	});

	test("Mit Prop 'icon = i-ri-add-line' wird das Header-Icon angezeigt und auf ein Plus gesetzt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { icon: 'i-ri-add-line' } });
		expect(wrapper.find(".header--icon").find("span").classes()).toContain("i-ri-add-line");
	});

	test("Mit Prop 'title = Some Title' wird der Titel im Header gesetzt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { title: 'Some Title' } });
		expect(wrapper.find(".header--title").text()).toBe("Some Title");
	});

	test("Mit Prop 'subtitle = Some Subtitle' wird der Untertitel im Header gesetzt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { subtitle: 'Some Subtitle' } });
		expect(wrapper.find(".header--subtitle").text()).toBe("Some Subtitle");
	});

	test("Mit Prop 'info = Some Info' wird der Infotext im Header gesetzt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { info: 'Some Info' } });
		expect(wrapper.find(".header--info").text()).toBe("Some Info");
	});

	test("Mit Prop 'content = Some Content' wird der Content gesetzt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { content: 'Some Content' } });
		expect(wrapper.find(".content--main").text()).toBe("Some Content");
	});

	test("Mit Prop 'showDivider = true' wird der Divider zwischen dem Footer und dem Content angezeigt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { showDivider: true } });
	...
	test("Mit Prop 'showDivider = false' wird der Divider zwischen dem Footer und dem Content nicht angezeigt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { showDivider: false } });
		expect(wrapper.find("hr").exists()).toBeFalsy();
	});

	test("Mit Prop 'footer = Some Footer' wird der Footer gesetzt ", () => {
		const wrapper = mount(SvwsUiCard, { props: { footer: 'Some Footer' } });
		expect(wrapper.find(".footer--main").text()).toBe("Some Footer");
	});
...
describe("PropHandhabung läuft korrekt für Buttons und Transitions", () => {
	...
	test("Mit Prop 'isOpen = true' wird die Card geöffnet dargestellt", async () => {
		const wrapper = mount(SvwsUiCard, { props: { isOpen: true } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		expect(wrapper.find(".svws-ui-card--header").classes()).toContain("svws-active");
		const style = wrapper.find(".svws-ui-card--body-wrapper").attributes("style");
		expect(style === undefined || !style.includes("display: none;")).toBeTruthy();
		expect((wrapper.vm as any).isActive).toBe(true);
		const body = wrapper.find(".svws-ui-card--body-wrapper").element as HTMLElement;
		expect(body.style.maxHeight).toBe("fit-content");
		const collapseIcon = wrapper.find(".header--collapse-icon").find("span");
	...
	test("Mit Prop 'isOpen = false' wird die Card geschlossen dargestellt", () => {
		const wrapper = mount(SvwsUiCard, { props: { isOpen: false } });
		expect(wrapper.find(".svws-ui-card--header").classes()).not.toContain("svws-active");
		const style = wrapper.find(".svws-ui-card--body-wrapper").attributes("style");
	...
	test("Mit Prop 'buttonContainer = content', 'buttonPosition = left' und Funktionen für alle 4 Buttons, werden diese auf der linken Seite des Contentbereichs "
		+ "angezeigt", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: "content", buttonPosition: "left", onEdit : () => {},	onSave : () => {},
			onCancel : () => {}, onDelete : () => {} }, attachTo: document.body });
		expect(wrapper.find("#svws-ui-card--button-content-left").exists()).toBeTruthy();
		expect(wrapper.find(".card--buttons").classes()).not.toContain("ml-auto");
		wrapper.unmount();
	});
	...
	test("Mit Prop 'buttonContainer = content', 'buttonPosition = right' und Funktionen für alle 4 Buttons, werden diese auf der rechten Seite des Contentbereichs "
		+ "angezeigt", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: "content", buttonPosition: "right", onEdit : () => {},	onSave : () => {},
			onCancel : () => {}, onDelete : () => {} }, attachTo: document.body });
		expect(wrapper.find("#svws-ui-card--button-content-right").exists()).toBeTruthy();
		expect(wrapper.find(".card--buttons").classes()).toContain("ml-auto");
		wrapper.unmount();
	});
	...
	test("Mit Prop 'buttonContainer = footer', 'buttonPosition = left' und Funktionen für alle 4 Buttons, werden diese auf der linken Seite des Footers "
		+ "angezeigt", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: "footer", buttonPosition: "left", onEdit : () => {},	onSave : () => {},
			onCancel : () => {}, onDelete : () => {} }, attachTo: document.body });
		expect(wrapper.find("#svws-ui-card--button-footer-left").exists()).toBeTruthy();
		expect(wrapper.find(".card--buttons").classes()).not.toContain("ml-auto");
		wrapper.unmount();
	});
	...
	test("Mit Prop 'buttonContainer = footer', 'buttonPosition = right' und Funktionen für alle 4 Buttons, werden diese auf der rechten Seite des Footers "
		+ "angezeigt", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: "footer", buttonPosition: "right", onEdit : () => {},	onSave : () => {},
			onCancel : () => {}, onDelete : () => {} }, attachTo: document.body });
		expect(wrapper.find("#svws-ui-card--button-footer-right").exists()).toBeTruthy();
		expect(wrapper.find(".card--buttons").classes()).toContain("ml-auto");
		wrapper.unmount();
	});

	test("Mit Prop 'buttonMode = text' und Funktionen für alle 4 Buttons erhält man Textbuttons" , () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: "text", onEdit : () => {},	onSave : () => {}, onCancel : () => {}, onDelete : () => {} }
			, attachTo: document.body });
		const buttonLabels = ["Speichern", "Bearbeiten", "Löschen", "Abbrechen"];
		const buttonWrapper = wrapper.find(".card--buttons");
	...
	test("Mit Prop 'buttonMode = icon' und Funktionen für alle 4 Buttons erhält man die Textbuttons Bearbeiten, Speichern, Löschen und Abbrechen" , () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: "icon", onEdit : () => {},	onSave : () => {}, onCancel : () => {}, onDelete : () => {} }
			, attachTo: document.body });
		...
		buttonWrapper.findAll(".card--button").forEach(button => {
	...
	test("Mit Prop 'buttonOrientation = vertical' und Funktionen für alle 4 Buttons erhält vertikal ausgerichtete Buttons" , () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonOrientation: "vertical", onEdit : () => {},	onSave : () => {}, onCancel : () => {}, onDelete : () => {} }
			, attachTo: document.body });
		const buttons = wrapper.find(".card--buttons");
	...
	test("Mit Prop 'buttonOrientation = horizontal' und Funktionen für alle 4 Buttons erhält vertikal ausgerichtete Buttons" , () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonOrientation: "horizontal", onEdit : () => {},	onSave : () => {}, onCancel : () => {}, onDelete : () => {} }
			, attachTo: document.body });
		const buttons = wrapper.find(".card--buttons");
	...
	test("Mit Prop 'editButtonDisabled = true' und 'onEdit = () => {}' wird der Bearbeiten-Button disabled" , () => {
		const wrapper = mount(SvwsUiCard, { props: { editButtonDisabled: true, onEdit : () => {} }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Bearbeiten");
	...
	test("Mit Prop 'saveButtonDisabled = true' und 'onSave = () => {}' wird der Speichern-Button disabled" , () => {
		const wrapper = mount(SvwsUiCard, { props: { saveButtonDisabled: true, onSave : () => {} }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Speichern");
		expect(button?.element.hasAttribute("disabled")).toBeTruthy();
		wrapper.unmount();
	});

	test("Mit Prop 'deleteButtonDisabled = true' und 'onDelete = () => {}' wird der Löschen-Button disabled" , () => {
		const wrapper = mount(SvwsUiCard, { props: { deleteButtonDisabled: true, onDelete : () => {} }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Löschen");
		expect(button?.element.hasAttribute("disabled")).toBeTruthy();
		wrapper.unmount();
	});

	test("Mit Prop 'cancelButtonDisabled = true' und 'onCancel = () => {}' wird der Abbrechen-Button disabled" , () => {
		const wrapper = mount(SvwsUiCard, { props: { cancelButtonDisabled: true, onCancel : () => {} }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Abbrechen");
		expect(button?.element.hasAttribute("disabled")).toBeTruthy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'editButtonDisabled = true' und 'onEdit = () => {}' und 'editButtonDisabledReason = Test' wird der Bearbeiten-Button disabled " +
		...
		const wrapper = mount(SvwsUiCard, { props: { editButtonDisabled: true, onEdit : () => {}, editButtonDisabledReason: "Test" }, attachTo: document.body });
	...
	test("Mit den Props 'saveButtonDisabled = true' und 'onSave = () => {}' und 'saveButtonDisabledReason = Test' wird der Speichern-Button disabled " +
			"und ein Tooltip mit der Begründung gerendert" , async () => {
		const wrapper = mount(SvwsUiCard, { props: { saveButtonDisabled: true, onSave : () => {}, saveButtonDisabledReason: "Test" }, attachTo: document.body });
	...
	test("Mit den Props 'deleteButtonDisabled = true' und 'onDelete = () => {}' und 'deleteButtonDisabledReason = Test' wird der Löschen-Button disabled " +
		"und ein Tooltip mit der Begründung gerendert" , async () => {
		const wrapper = mount(SvwsUiCard, { props: { deleteButtonDisabled: true, onDelete : () => {}, deleteButtonDisabledReason: "Test" }, attachTo: document.body });
	...
	"und ein Tooltip mit der Begründung gerendert" , async () => {
		const wrapper = mount(SvwsUiCard, { props: { cancelButtonDisabled: true, onCancel : () => {}, cancelButtonDisabledReason: "Test" }, attachTo: document.body });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
	...
	test("Mit den Props 'buttonMode = icon' und  'onEdit = () => {}' wird um den Bearbeiten-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onEdit : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = icon' und  'onSave = () => {}' wird um den Speichern-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onSave : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = icon' und  'onDelete = () => {}' wird um den Löschen-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onDelete : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = icon' und  'onCancel = () => {}' wird um den Abbrechen-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onCancel : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = icon' und  'onEdit = () => {}' und 'editButtonDisabled: true' wird um den Bearbeiten-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onEdit : () => {}, editButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("span").classes().includes("i-ri-edit-2-line"))?.exists()).toBeTruthy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = icon' und  'onSave = () => {}' und 'saveButtonDisabled: true' wird um den Speichern-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onSave : () => {}, saveButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("span").classes().includes("i-ri-check-line"))?.exists()).toBeTruthy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = icon' und  'onDelete = () => {}' und 'deleteButtonDisabled: true' wird um den Löschen-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onDelete : () => {}, deleteButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("span").classes().includes("i-ri-delete-bin-line"))?.exists()).toBeTruthy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = icon' und  'onCancel = () => {}' und 'cancelButtonDisabled: true' wird um den Abbrechen-Button ein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon', onCancel : () => {}, cancelButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("span").classes().includes("i-ri-close-line"))?.exists()).toBeTruthy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = text' und  'onEdit = () => {}' wird um den Bearbeiten-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onEdit : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = text' und  'onSave = () => {}' wird um den Speichern-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onSave : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = text' und  'onDelete = () => {}' wird um den Löschen-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onDelete : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = text' und  'onCancel = () => {}' wird um den Abbrechen-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onCancel : () => {} }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = text' und  'onEdit = () => {}' und 'editButtonDisabled: true' wird um den Bearbeiten-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onEdit : () => {}, editButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("button").text() === "Bearbeiten")?.exists()).toBeFalsy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = text' und  'onSave = () => {}' und 'saveButtonDisabled: true' wird um den Speichern-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onSave : () => {}, saveButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("button").text() === "Speichern")?.exists()).toBeFalsy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = text' und  'onDelete = () => {}' und 'deleteButtonDisabled: true' wird um den Löschen-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onDelete : () => {}, deleteButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("button").text() === "Löschen")?.exists()).toBeFalsy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = text' und  'onCancel = () => {}' und 'cancelButtonDisabled: true' wird um den Abbrechen-Button kein Tooltip gerendert", () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onCancel : () => {}, cancelButtonDisabled: true }, attachTo: document.body });
		expect(wrapper.findAll(".tooltip-trigger").find(tooltip => tooltip.find("button").text() === "Abbrechen")?.exists()).toBeFalsy();
		wrapper.unmount();
	});
	...
	test("Mit den Props 'buttonMode = text' und  'onEdit = () => {}' und 'editButtonDisabled: true' und 'editButtonDisabledReason: Test' wird um den "
		...
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onEdit : () => {}, editButtonDisabled: true, editButtonDisabledReason: 'Test' }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = text' und  'onSave = () => {}' und 'saveButtonDisabled: true' und 'saveButtonDisabledReason: Test' wird um den "
		...
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onSave : () => {}, saveButtonDisabled: true, saveButtonDisabledReason: 'Test' }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = text' und  'onDelete = () => {}' und 'deleteButtonDisabled: true' und 'deleteButtonDisabledReason: Test' wird um den "
		...
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onDelete : () => {}, deleteButtonDisabled: true, deleteButtonDisabledReason: 'Test' }, attachTo: document.body });
	...
	test("Mit den Props 'buttonMode = text' und  'onCancel = () => {}' und 'cancelButtonDisabled: true' und 'cancelButtonDisabledReason: Test' wird um den "
		...
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text', onCancel : () => {}, cancelButtonDisabled: true, cancelButtonDisabledReason: 'Test' }, attachTo: document.body });
	...
	test.each(buttonTypes)("Mit Props Button Definitionen für %s und 'buttonMode' = 'text' hat der Button folgenden type: %s",
		async (functions, type, functionProps) => {
			const wrapper = mount(SvwsUiCard, {	props: { buttonMode: 'text', ...functionProps }, attachTo: document.body });
	...
	test.each(buttonIconTypes)("Mit Props Button Definitionen für %s und 'buttonMode' = 'icon' hat der Button den type 'icon' und span die Klasse: %s",
		async (functions, type, functionProps) => {
			const wrapper = mount(SvwsUiCard, {	props: { buttonMode: 'icon', ...functionProps }, attachTo: document.body });
			const button = wrapper.findComponent(SvwsUiButton);
	...
	test.each(simpleButtons)("Mit Button Definitionen für %s, 'buttonMode' = 'text' und 'compact' = true ist die size des Buttons gleich small", () => {
		const wrapper = mount(SvwsUiCard, {	props: { buttonMode: 'text', onEdit: () => {}, compact: true}, attachTo: document.body });
		const button = wrapper.findComponent(SvwsUiButton);
	...
	test.each(simpleButtons)("Mit Button Definitionen für %s, 'buttonMode' = 'text' und 'compact' = false ist die size des Buttons gleich normal", () => {
		const wrapper = mount(SvwsUiCard, {	props: { buttonMode: 'text', onEdit: () => {}, compact: false}, attachTo: document.body });
		const button = wrapper.findComponent(SvwsUiButton);
	...
	test.each(simpleButtons)("Mit Button Definitionen für %s, 'buttonMode' = 'icon' und 'compact' = false ist die size des Buttons gleich normal", () => {
		const wrapper = mount(SvwsUiCard, {	props: { buttonMode: 'icon', onEdit: () => {}, compact: false}, attachTo: document.body });
		const button = wrapper.findComponent(SvwsUiButton);
		expect(button.props().size).toBe('normal');
	});
	...
	test.each(simpleButtons)("Mit Button Definitionen für %s, 'buttonMode' = 'icon' und 'compact' = true ist die size des Buttons gleich normal", () => {
		const wrapper = mount(SvwsUiCard, {	props: { buttonMode: 'icon', onEdit: () => {}, compact: true}, attachTo: document.body });
		const button = wrapper.findComponent(SvwsUiButton);
		expect(button.props().size).toBe('normal');
	});
});
...
describe("Events laufen korrekt", () => {
	test("Klick auf Header (geschlossene Card) öffnet die Card", async () => {
		const wrapper = mount(SvwsUiCard, { props: { isOpen: false }, global: { stubs: { transition: false} } });
		const header = wrapper.find(".svws-ui-card--header");
		const body = wrapper.find(".svws-ui-card--body-wrapper")
		...
		expect((wrapper.vm as any).isActive).toBe(true);
		expect(header.classes()).toContain("svws-active");
		expect(body.attributes("style")).not.toContain("display: none");
	});

	test("Klick auf Header (geöffnete Card) schließt die Card", async () => {
		const wrapper = mount(SvwsUiCard, { props: { isOpen: true }, global: { stubs: { transition: false} } });
		const header = wrapper.find(".svws-ui-card--header");
		const body = wrapper.find(".svws-ui-card--body-wrapper")
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		expect(header.classes()).toContain("svws-active");
...
describe.concurrent("Alle computeds berechnen korrekt", () => {
	...
	test("computed->showCollapseIconLeft liefert true, wenn kein collapseLeft-slot gesetzt ist, die Card collapsible ist und das Collapse-icon links ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, collapseIconPosition: 'left' } });
		expect( await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showCollapseIconLeft).toBeTruthy();
	});

	test("computed->showCollapseIconLeft liefert false, wenn die Card nicht collapsible ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: false, collapseIconPosition: 'left' } });
		expect( await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showCollapseIconLeft).toBeFalsy();
	});
	...
	test("computed->showCollapseIconLeft liefert false, wenn ein collapseLeft-slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, {props: { collapsible: true }, slots:{ collapseLeft: "Custom Slot Inhalt" }});
		expect( await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showCollapseIconLeft).toBeFalsy();
	});
	...
	test("computed->showCollapseIconRight liefert true, wenn kein collapseRight-slot gesetzt ist, die Card collapsible ist und das Collapse-icon rechts ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, collapseIconPosition: 'right' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showCollapseIconRight).toBeTruthy();
	});

	test("computed->showCollapseIconRight liefert false, wenn die Card nicht collapsible ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: false, collapseIconPosition: 'right' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showCollapseIconRight).toBeFalsy();
	});
	...
	test("computed->showCollapseIconRight liefert false, wenn ein collapseRight-slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true }, slots: { collapseRight: "Custom Slot Inhalt" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showCollapseIconRight).toBeFalsy();
	});
	...
	test("computed->showIcon liefert true, wenn kein icon-Slot gesetzt ist, ein Icon über Props definiert ist und die Länge des Icons nicht 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { icon: 'some-icon' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showIcon).toBeTruthy();
	});

	test("computed->showIcon liefert false, wenn ein icon-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { icon: 'some-icon' }, slots: { icon: "Custom Icon Inhalt" }});
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showIcon).toBeFalsy();
	});
	...
	test("computed->showIcon liefert false, wenn die Länge des Icons 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { icon: '' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showIcon).toBeFalsy();
	});

	// showTitleWrapper
	test("computed->showTitleWrapper liefert true, wenn ein title-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { title: "Custom Title Inhalt" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showTitleWrapper).toBeTruthy();
	});

	test("computed->showTitleWrapper liefert true, wenn ein subtitle-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { subtitle: "Custom Subtitle Inhalt" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showTitleWrapper).toBeTruthy();
	});

	test("computed->showTitleWrapper liefert true, wenn ein title-Prop definiert ist und die Länge des Titels nicht 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { title: "A Title" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showTitleWrapper).toBeTruthy();
	});
	...
	test("computed->showTitle liefert true, wenn kein title-Slot gesetzt ist, das title-Prop definiert ist und die Länge des Titels nicht 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { title: "A Title" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showTitle).toBeTruthy();
	});

	test("computed->showTitle liefert false, wenn ein title-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { title: "Custom Title Inhalt" }, props: { title: "A Title" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showTitle).toBeFalsy();
	});
	...
	test("computed->showSubtitle liefert true, wenn kein subtitle-Slot gesetzt ist, das subtitle-Prop definiert ist und die Länge des Subtitels nicht 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { subtitle: "A Subtitle" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showSubtitle).toBeTruthy();
	});

	test("computed->showSubtitle liefert false, wenn ein subtitle-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { subtitle: "Custom Subtitle Inhalt" }, props: { subtitle: "A Subtitle" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showSubtitle).toBeFalsy();
	});
	...
	test("computed->showInfo liefert true, wenn kein info-Slot gesetzt ist, das info-Prop definiert ist und die Länge des Info-Werts nicht 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { info: "Some Info" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showInfo).toBeTruthy();
	});

	test("computed->showInfo liefert false, wenn ein info-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { info: "Custom Info Inhalt" }, props: { info: "Some Info" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showInfo).toBeFalsy();
	});
	...
	test("computed->showInfo liefert false, wenn das info-Prop gesetzt ist, aber die Länge des Info-Werts 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { info: "" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showInfo).toBeFalsy();
	});
	...
	test("computed->showContentMain liefert true, wenn kein content-Slot gesetzt ist, das content-Prop definiert ist und die Länge des Inhalts nicht 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { content: "Main Content" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentMain).toBeTruthy();
	});

	test("computed->showContentMain liefert false, wenn ein content-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { content: "Custom Content Inhalt" }, props: { content: "Main Content" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentMain).toBeFalsy();
	});
	...
	test("computed->showContentMain liefert false, wenn das content-Prop gesetzt ist, aber die Länge des Inhalts 0 ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { content: "" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentMain).toBeFalsy();
	});
	...
	test("computed->showContentLeftButton liefert true, wenn kein buttonContentLeft-Slot gesetzt ist, Buttons definiert sind, die Buttonposition links ist und der Buttoncontainer content ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonPosition: 'left', buttonContainer: 'content', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentLeftButton).toBeTruthy();
	});

	test("computed->showContentLeftButton liefert false, wenn ein buttonContentLeft-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { buttonContentLeft: "Custom Button Content Left" }, props: { buttonPosition: 'left', buttonContainer: 'content', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentLeftButton).toBeFalsy();
	});

	test("computed->showContentLeftButton liefert false, wenn keine Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonPosition: 'left', buttonContainer: 'content' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentLeftButton).toBeFalsy();
	});

	test("computed->showContentLeftButton liefert false, wenn buttonPosition nicht links ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonPosition: 'right', buttonContainer: 'content', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentLeftButton).toBeFalsy();
	});

	test("computed->showContentLeftButton liefert false, wenn buttonContainer nicht content ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonPosition: 'left', buttonContainer: 'footer', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentLeftButton).toBeFalsy();
	});
	...
	test("computed->showContentRightButton liefert true, wenn kein buttonContentRight-Slot gesetzt ist, Buttons definiert sind, der Buttonposition rechts ist und der Buttoncontainer content ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonPosition: 'right', buttonContainer: 'content', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentRightButton).toBeTruthy();
	});

	test("computed->showContentRightButton liefert false, wenn ein buttonContentRight-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { buttonContentRight: "Custom Button Content Right" }, props: { buttonPosition: 'right', buttonContainer: 'content', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentRightButton).toBeFalsy();
	});

	test("computed->showContentRightButton liefert false, wenn keine Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonPosition: 'right', buttonContainer: 'content' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentRightButton).toBeFalsy();
	});
	...
	test("computed->showContentRightButton liefert false, wenn buttonContainer nicht content ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonPosition: 'right', buttonContainer: 'footer', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showContentRightButton).toBeFalsy();
	});
	...
	test("computed->showFooterDivider liefert true, wenn kein footerDivider-Slot gesetzt ist und die Prop showDivider true ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { showDivider: true } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterDivider).toBeTruthy();
	});

	test("computed->showFooterDivider liefert false, wenn ein footerDivider-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { footerDivider: "Custom Divider Content" }, props: { showDivider: true } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterDivider).toBeFalsy();
	});
	...
	test("computed->showFooter liefert true, wenn ein footer-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { footer: "Custom Footer Content" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeTruthy();
	});

	test("computed->showFooter liefert true, wenn ein buttonFooterLeft-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { buttonFooterLeft: "Custom Button Footer Left" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeTruthy();
	});

	test("computed->showFooter liefert true, wenn ein buttonFooterRight-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { buttonFooterRight: "Custom Button Footer Right" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeTruthy();
	});

	test("computed->showFooter liefert true, wenn props.footer gesetzt und nicht leer ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { footer: "Non-empty footer" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeTruthy();
	});

	test("computed->showFooter liefert true, wenn props.buttonContainer 'footer' ist und Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'footer', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeTruthy();
	});
	...
	test("computed->showFooter liefert false, wenn kein footer-Slot und kein buttonFooterSlot gesetzt ist, wenn keine Buttons definiert sind und props.footer leer ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { footer: "" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeFalsy();
	});
	...
	test("computed->showFooter liefert false, wenn kein footer-Slot und kein buttonFooterSlot gesetzt ist, wenn Buttons definiert sind, aber im Content angezeigt werden und props.footer leer ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { footer: "", buttonContainer: 'content', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeFalsy();
	});
	...
	test("computed->showFooter liefert false, wenn kein footer-Slot und kein buttonFooterSlot gesetzt ist, wenn Buttons definiert sind, aber im Content angezeigt werden und props.footer undefiniert ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'content', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooter).toBeFalsy();
	});
	...
	test("computed->showFooterRightButton liefert true, wenn kein buttonFooterRight-Slot gesetzt ist, Buttons definiert sind, der Buttoncontainer footer ist und der Buttonposition rechts ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'footer', buttonPosition: 'right', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterRightButton).toBeTruthy();
	});

	test("computed->showFooterRightButton liefert false, wenn ein buttonFooterRight-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { buttonFooterRight: "Custom Button Footer Right" }, props: { buttonContainer: 'footer', buttonPosition: 'right', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterRightButton).toBeFalsy();
	});

	test("computed->showFooterRightButton liefert false, wenn keine Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'footer', buttonPosition: 'right' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterRightButton).toBeFalsy();
	});

	test("computed->showFooterRightButton liefert false, wenn buttonContainer nicht footer ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'content', buttonPosition: 'right', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterRightButton).toBeFalsy();
	});
	...
	test("computed->showFooterMain liefert true, wenn kein footer-Slot gesetzt ist und props.footer nicht leer ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { footer: "Non-empty footer" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterMain).toBeTruthy();
	});

	test("computed->showFooterMain liefert false, wenn ein footer-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { footer: "Custom Footer Content" }, props: { footer: "Non-empty footer" } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterMain).toBeFalsy();
	});
	...
	test("computed->showFooterMain liefert false, wenn props.footer nicht gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: {} });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterMain).toBeFalsy();
	});
	...
	test("computed->showFooterLeftButton liefert true, wenn kein buttonFooterLeft-Slot gesetzt ist, Buttons definiert sind, der Buttoncontainer footer ist und der Buttonposition links ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'footer', buttonPosition: 'left', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterLeftButton).toBeTruthy();
	});

	test("computed->showFooterLeftButton liefert false, wenn ein buttonFooterLeft-Slot gesetzt ist", async () => {
		const wrapper = mount(SvwsUiCard, { slots: { buttonFooterLeft: "Custom Button Footer Left" }, props: { buttonContainer: 'footer', buttonPosition: 'left', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterLeftButton).toBeFalsy();
	});

	test("computed->showFooterLeftButton liefert false, wenn keine Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'footer', buttonPosition: 'left' } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterLeftButton).toBeFalsy();
	});

	test("computed->showFooterLeftButton liefert false, wenn buttonContainer nicht footer ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonContainer: 'content', buttonPosition: 'left', onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showFooterLeftButton).toBeFalsy();
	});

	// showButtons
	test("computed->showButtons liefert true, wenn Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard, { props: { onEdit: () => {} }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showButtons).toBeTruthy();
	});

	test("computed->showButtons liefert false, wenn keine Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard);
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.showButtons).toBeFalsy();
	});

	// buttonContainerId
	test("computed->buttonContainerId liefert undefined, wenn keine Buttons definiert sind", async () => {
		const wrapper = mount(SvwsUiCard);
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.buttonContainerId).toBeUndefined();
	});

	test("computed->buttonContainerId liefert '#svws-ui-card--button-content-left', wenn Buttons definiert sind und buttonContainer content und buttonPosition links ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { onEdit: () => {}, buttonContainer: 'content', buttonPosition: 'left' }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.buttonContainerId).toBe("#svws-ui-card--button-content-left");
	});

	test("computed->buttonContainerId liefert '#svws-ui-card--button-content-right', wenn Buttons definiert sind und buttonContainer content und buttonPosition rechts ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { onEdit: () => {}, buttonContainer: 'content', buttonPosition: 'right' }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.buttonContainerId).toBe("#svws-ui-card--button-content-right");
	});

	test("computed->buttonContainerId liefert '#svws-ui-card--button-footer-left', wenn Buttons definiert sind und buttonContainer footer und buttonPosition links ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { onEdit: () => {}, buttonContainer: 'footer', buttonPosition: 'left' }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.buttonContainerId).toBe("#svws-ui-card--button-footer-left");
	});

	test("computed->buttonContainerId liefert '#svws-ui-card--button-footer-right', wenn Buttons definiert sind und buttonContainer footer und buttonPosition rechts ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { onEdit: () => {}, buttonContainer: 'footer', buttonPosition: 'right' }, attachTo: document.body });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.buttonContainerId).toBe("#svws-ui-card--button-footer-right");
	});

	// ariaExpanded
	test("computed->ariaExpanded liefert true, wenn collapsible true ist und isActive true ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, isOpen: true } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.isActive).toBe(true);
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.ariaExpanded).toBe(true);
	});

	test("computed->ariaExpanded liefert false, wenn collapsible true ist und isActive false ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true, isOpen: false } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.isActive).toBe(false);
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.ariaExpanded).toBe(false);
	});

	test("computed->ariaExpanded liefert undefined, wenn collapsible false ist und isActive true ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: false, isOpen: true } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.isActive).toBe(true);
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.ariaExpanded).toBeUndefined();
	});

	test("computed->ariaExpanded liefert undefined, wenn collapsible false ist und isActive false ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: false, isOpen: false } });
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.isActive).toBe(false);
		expect(await wrapper.findComponent({ name: "SvwsUiCard" }).vm.ariaExpanded).toBeUndefined();
	});
	...
	test.each(buttonCombinations)(
		...
		async (functions, buttonLabels, functionProps) => {
			const wrapper = mount(SvwsUiCard, {	props: { ...functionProps }, attachTo: document.body });
			const buttons = await wrapper.findComponent({ name: "SvwsUiCard" }).vm.buttons;
...
describe.concurrent("Slots werden korrekt gerendert", () => {
	const slots = [
		...
		{ name: 'content', content: 'Slot Inhalt für content' },
	...
	test.each(slots)("Slot $name wird korrekt gerendert", ({ name, content }) => {
		const wrapper = mount(SvwsUiCard, { slots: { [name]: content } });
...
describe.concurrent("Funktionen werden korrekt ausgeführt", async () => {
	...
	test("function-> openCard (Card noch geschlossen) setzt die maxHeight der Card auf die scrollHeight", async () => {
		...
		const bodyWrapper = wrapper.find(".svws-ui-card--body-wrapper");
	...
	test("function-> tooltipDisabled gibt false zurück, wenn der 'buttonMode: icon' entspricht", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'icon' } });
	...
	test("function-> tooltipDisabled gibt false zurück, wenn der Button disabled ist und ein 'disabledReason' angegeben ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { buttonMode: 'text' } });
		const tooltipDisabled = await wrapper.findComponent({
			name: "SvwsUiCard",
		}).vm.tooltipDisabled;
		const result = tooltipDisabled({
...
describe("Übergebene Button-Funktionen werden korrekt ausgeführt", async () => {
	test("Die übergebene onEdit Funktion wird bei Klick auf den Edit Button ausgeführt", async () => {
		...
		const wrapper = mount(SvwsUiCard, { props: { onEdit: onEditFunction }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Bearbeiten");
		if(button === undefined)
	...
	test("Die übergebene onSave Funktion wird bei Klick auf den Save Button ausgeführt", async () => {
		...
		const wrapper = mount(SvwsUiCard, { props: { onSave: onSaveFunction }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Speichern");
		if(button === undefined)
			fail("Button darf nicht undefined sein");
		await button.trigger("click");
	...
	test("Die übergebene onDelete Funktion wird bei Klick auf den Delete Button ausgeführt", async () => {
		...
		const wrapper = mount(SvwsUiCard, { props: { onDelete: onDeleteFunction }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Löschen");
		if(button === undefined)
			fail("Button darf nicht undefined sein");
		await button.trigger("click");
	...
	test("Die übergebene onCancel Funktion wird bei Klick auf den Cancel Button ausgeführt", async () => {
		...
		const wrapper = mount(SvwsUiCard, { props: { onCancel: onCancelFunction }, attachTo: document.body });
		const button = wrapper.find(".card--buttons").findAll("button").find(button => button.text() === "Abbrechen");
		if(button === undefined)
			fail("Button darf nicht undefined sein");
		await button.trigger("click");
...
describe("Aria Attribute werden korrekt gesetzt", async () => {
	...
	test.each(labelButtons)("Mit Button Definitionen für %s ist das aria-label= '%s'", async (functions, label: string, functionProps) => {
		const wrapper = mount(SvwsUiCard, {	props: { buttonMode: 'icon', compact: true, ...functionProps }, attachTo: document.body });
	...
	test("Das Attribut aria-expanded im Header ist true, wenn das computed ariaExpanded true ist", async () => {
		...
		const wrapper = mount(SvwsUiCard, { props: { isOpen: true } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		const ariaExpanded = wrapper.find(".svws-ui-card--header").attributes("aria-expanded");
	...
	test("Das Attribut aria-controls im Header besteht aus dem String 'cardBody' und der InstanzID, wenn 'collapsible' = true ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: true } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		const ariaControls = wrapper.find(".svws-ui-card--header").attributes("aria-controls");
		const instanceId = await wrapper.findComponent({ name: "SvwsUiCard" }).vm.instanceId;
	...
	test("Das Attribut aria-controls im Header ist undefined, wenn 'collapsible' = false ist", async () => {
		const wrapper = mount(SvwsUiCard, { props: { collapsible: false } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		const ariaControls = wrapper.find(".svws-ui-card--header").attributes("aria-controls");
	...
	test("Das Attribut aria-label im collapse-Icon links ist 'Card geschlossen', wenn 'isActive' = false ist", async () => {
		...
		const wrapper = mount(SvwsUiCard, { props: { isOpen: false, collapseIconPosition: 'left' } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		const ariaControls = wrapper.find(".header--collapse-icon").find("span").attributes("aria-label");
		expect(ariaControls).toBeDefined();
	...
	test("Das Attribut aria-label im collapse-Icon links ist 'Card geöffnet', wenn 'isActive' = true ist ", async () => {
		...
		const wrapper = mount(SvwsUiCard, { props: { isOpen: true, collapseIconPosition: 'left' } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		const ariaControls = wrapper.find(".header--collapse-icon").find("span").attributes("aria-label");
		expect(ariaControls).toBeDefined();
	...
	test("Das Attribut aria-label im collapse-Icon rechts ist 'Card geschlossen', wenn 'isActive' = false ist", async () => {
		// Der Wert von isActive wird durch isOpen beeinflusst und in diesem Fall false
		const wrapper = mount(SvwsUiCard, { props: { isOpen: false, collapseIconPosition: 'right' } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		const ariaControls = wrapper.find(".header--collapse-icon").find("span").attributes("aria-label");
		expect(ariaControls).toBeDefined();
		expect(ariaControls).toBe("Card geschlossen");
	...
	test("Das Attribut aria-label im collapse-Icon rechts ist 'Card geöffnet', wenn 'isActive' = true ist ", async () => {
		// Der Wert von isActive wird durch isOpen beeinflusst und in diesem Fall true
		const wrapper = mount(SvwsUiCard, { props: { isOpen: true, collapseIconPosition: 'right' } });
		await wrapper.findComponent({ name: "SvwsUiCard" }).vm.$nextTick();
		const ariaControls = wrapper.find(".header--collapse-icon").find("span").attributes("aria-label");
		expect(ariaControls).toBeDefined();
		expect(ariaControls).toBe("Card geöffnet");
