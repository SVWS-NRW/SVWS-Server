...
| required              | `boolean`                     | `false`                                                | Definiert, ob das Select ein Pflichtfeld ist.<br/> Fügt automatisch einen Validator hinzu, wenn dieser nicht durch `skipDefaultValidation` übersprungen wird.                                       |
...
| validation            | `() => List<ValidatorFehler>` | leere `ArrayList<ValidatorFehler>`                     | Eine Liste von Validatorfehlern, die bei Überprüfung der selektierten Optionen aufgetreten sind.                                                                                                    |
| skipDefaultValidation | `boolean`                     | `false`                                                | Bestimmt, ob der Defaultvalidator für `required = true` übersprungen werden soll (zum Beispiel weil man das Styling haben will, aber die Validierung über einen eigenen Validator vorgenommen wird) |
