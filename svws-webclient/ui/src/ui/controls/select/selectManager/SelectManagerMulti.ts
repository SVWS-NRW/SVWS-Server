import { type BaseSelectManagerMultiConfig, BaseSelectManagerMulti } from "./BaseSelectManagerMulti";

export interface SelectManagerMultiConfig<T> extends BaseSelectManagerMultiConfig<T> {
	selectionDisplayText?: (option: T) => string;
	optionDisplayText?: (option: T) => string;
}

/**
 * Einfache Manager Klasse zur Verwendung in einer Multi-Select-Komponente (UiSelect.vue). SelectManager übernehmen die Logik der Select-Komponente.
 ...
 */
export class SelectManagerMulti<T> extends BaseSelectManagerMulti<T> {
	...
	public constructor (config?: SelectManagerMultiConfig<T>) {
		super(config);
		if (config === undefined)
			return;
