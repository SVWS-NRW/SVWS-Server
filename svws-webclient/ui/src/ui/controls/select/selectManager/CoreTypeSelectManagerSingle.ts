...
export interface CoreTypeSelectManagerSingleConfig<T extends CoreTypeData, U extends CoreType<T, U>> extends Omit<BaseSelectManagerSingleConfig<T>, 'options'> {
	clazz?: Class<U>;
...
export class CoreTypeSelectManagerSingle<T extends CoreTypeData, U extends CoreType<T, U>> extends BaseSelectManagerSingle<T> {
	...
	public constructor(config?: CoreTypeSelectManagerSingleConfig<T, U>) {
		if (config === undefined) {
			super(config);
			return;
		}
		...
		const baseConfig: BaseSelectManagerSingleConfig<T> = {
	...
	/**
		 ...
		 */
	public setConfig(config: CoreTypeSelectManagerSingleConfig<T, U>) {
		if (config.clazz !== undefined)
