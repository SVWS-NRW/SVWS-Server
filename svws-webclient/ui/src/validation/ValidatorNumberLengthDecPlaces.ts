 ...
 */
export class ValidatorNumberLengthDecPlaces extends BasicValidator {
	...
	/**
	 * Erzeugt einen neuen Validator mit der Funktion zum Zugriff auf die Zahl
	 *
	 ...
	 */
	constructor(data: () => number | null | undefined, maxLen?: number | null, minLen?: number | null) {
	...
	protected pruefe(): boolean {
		...
		if ((data === undefined) || (data === null) || (data === 0)) {
			if ((this.minLen !== null) && (this.minLen > 0)) {
				this.addFehler(0, "Der Wert muss angeben sein und darf nicht leer sein. Weiterhin muss die minimale Länge der Nachkommastellen " + this.minLen + " Zeichen betragen.");
				return false;
			}
			return true;
		}

		const [...nachkomma] = String(data).split('.');

		// Prüfe die minimale Länge des Strings
		const len = nachkomma.length;
		if ((this.minLen !== null) && (len < this.minLen)) {
			this.addFehler(0, "Der Wert muss mindestens " + this.minLen + " Nachkommastellen haben.");
			return false;
		}

		// Prüfe die maximale Länge des Strings
		if ((this.maxLen !== null) && (len > this.maxLen)) {
			this.addFehler(0, "Der Wert darf maximal " + this.maxLen + " Nachkommastellen haben.");
			return false;
		}
