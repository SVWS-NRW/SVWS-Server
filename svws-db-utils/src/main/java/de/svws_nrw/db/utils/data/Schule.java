...
public class Schule {
	 ...
	 */
	public static synchronized Schule queryCached(final DBEntityManager conn) {
		final Schule schule = cache.get(conn.getDBSchema());
	 ...
	 *         falls diese nicht aus der DB gelesen werden konnte
	 */
	public static synchronized Schule query(final DBEntityManager conn) {
		final DTOEigeneSchule dto = conn.querySingle(DTOEigeneSchule.class);
		if (dto == null)
			return null;
		final DTOSchuljahresabschnitte dtoAbschnitt = conn.queryByKey(DTOSchuljahresabschnitte.class, dto.Schuljahresabschnitts_ID);
		if (dtoAbschnitt == null)
			return null;
		final Schule schule = new Schule(dto, dtoAbschnitt);
	 ...
	 */
	public Schule(final DTOEigeneSchule dto, final DTOSchuljahresabschnitte dtoAbschnitt) {
		this.dto = dto;
