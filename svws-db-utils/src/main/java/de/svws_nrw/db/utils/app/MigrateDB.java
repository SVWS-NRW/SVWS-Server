...
public class MigrateDB {
	...
	public static void main(final String[] args) {
		...
		try {
			...
			cmdLine.addOption(new CommandLineOption("d", "developerMode", false,
					"Führt den Import im Developer-Mode durch. Dies bedeutet, dass die Datenbank nur für Testzwecke geeignet ist und entsprechend gekennzeichnet wird."));
			cmdLine.addOption(new CommandLineOption("n", "schulNummer", true,
					"Für Schildzentral-Quell-DBs: Gibt die Schulnummer an, für welche Daten aus der Quelldatenbank migriert werden sollen."));
			...
			try (Scanner scan = new Scanner(System.in)) {
				...
				if (!hinweis_akzpetiert) {
					hinweis_akzpetiert = konsoleFrageJaNein("Die Zieldatenbank muss für eine problemlose Migration neu angelegt werden. Dabei gehen alle"
							+ " Daten in der Ziel-Datenbank verloren. Fortfahren? (J/N) ", scan);
					if (!hinweis_akzpetiert) {
