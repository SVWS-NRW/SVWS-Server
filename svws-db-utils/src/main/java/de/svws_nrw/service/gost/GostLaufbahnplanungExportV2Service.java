...
public class GostLaufbahnplanungExportV2Service {
	 ...
	 * @param gostSchuelerRepository                    das Repository für den Zugriff auf die Schülerdaten für die gymnasiale Oberstufe
	 ...
	 * @param schuelerCredentialsService                der Service für den Zugriff auf die Schüler-Credentials
	 ...
	 */
	public GostLaufbahnplanungExportV2 get(final long idSchueler) {
	 ...
	 */
	public List<GostLaufbahnplanungExportV2> getList(final Collection<Long> idsSchueler) {
		return transactional(() -> {
			...
			final Map<Integer, List<DTOGostKlausurenVorgaben>> mapGostKlausurvorgaben = gostKlausurenVorgabeRepository.getMapByAbiturjahrgaenge(abiturjahrgaenge);
			...
			final List<GostLaufbahnplanungExportV2> result = new ArrayList<>();
			for (final Abiturdaten abidaten : listAbiturdaten) {
				...
				final List<DTOGostKlausurenVorgaben> klausurvorgaben = mapGostKlausurvorgaben.getOrDefault(abidaten.abiturjahr, new ArrayList<>());
				final DTOSchueler schueler = mapSchueler.get(abidaten.schuelerID);
				DTOGostSchueler gostSchueler = mapGostSchueler.get(abidaten.schuelerID);
				if (gostSchueler == null) {
					gostSchueler = new DTOGostSchueler(abidaten.schuelerID, false);
					gostSchuelerRepository.update(gostSchueler);
				}
				final AES aes = mapAES.get(abidaten.schuelerID);
				result.add(getExportInternal(abidaten, schule, jahrgangsdaten, listBeratungslehrer, gostFaecher, kombis, klausurvorgaben, schueler, gostSchueler, aes));
			}
			return result;
		});
	}
	...
	private static GostLaufbahnplanungExportV2 getExportInternal(final Abiturdaten abidaten, final SchuleStammdaten schule, final DTOGostJahrgangsdaten jahrgangsdaten,
			...
			final List<DTOGostKlausurenVorgaben> klausurvorgaben, final DTOSchueler schueler, final DTOGostSchueler gostSchueler, final AES aes) {
		final GostLaufbahnplanungExportV2 daten = new GostLaufbahnplanungExportV2();
		...
		for (final GostBeratungslehrer beratungslehrer : listBeratungslehrer) {
			final GostLaufbahnplanungExportV2Beratungslehrer exportBeratungslehrer = new GostLaufbahnplanungExportV2Beratungslehrer();
		...
		for (final GostFach fach : gostFaecher.faecher()) {
			final GostLaufbahnplanungExportV2Fach exportFach = new GostLaufbahnplanungExportV2Fach();
			...
			exportFach.istMoeglich[0] = fach.istMoeglichEF1;
			exportFach.istMoeglich[1] = fach.istMoeglichEF2;
			exportFach.istMoeglich[2] = fach.istMoeglichQ11;
			exportFach.istMoeglich[3] = fach.istMoeglichQ12;
			exportFach.istMoeglich[4] = fach.istMoeglichQ21;
			exportFach.istMoeglich[5] = fach.istMoeglichQ22;
			exportFach.wochenstundenQualifikationsphase = 3;
			exportFach.referenzfach1ID = fach.projektKursLeitfach1ID;
			exportFach.referenzfach2ID = fach.projektKursLeitfach2ID;
			daten.faecher.add(exportFach);
		}
		for (final DTOGostJahrgangFachkombinationen kombi : kombis) {
			final GostLaufbahnplanungExportV2Fachkombination exportKombi = new GostLaufbahnplanungExportV2Fachkombination();
		...
		final GostLaufbahnplanungExportV2Schueler schuelerDaten = new GostLaufbahnplanungExportV2Schueler();
		...
		// TODO schuelerDaten.istG8 bei Bedarf ggf. über den aktuellen SchuelerLernabschnitt bestimmen
		schuelerDaten.gkl[0] = gostSchueler.GKL_EF_AF1_Klausurvorgabe_ID;
		schuelerDaten.gkl[1] = gostSchueler.GKL_EF_AF2_Klausurvorgabe_ID;
		schuelerDaten.gkl[2] = gostSchueler.GKL_EF_AF3_Klausurvorgabe_ID;
		schuelerDaten.gkl[3] = gostSchueler.GKL_Q_AF1_Klausurvorgabe_ID;
		schuelerDaten.gkl[4] = gostSchueler.GKL_Q_AF2_Klausurvorgabe_ID;
		schuelerDaten.gkl[5] = gostSchueler.GKL_Q_AF3_Klausurvorgabe_ID;
		...
		for (final AbiturFachbelegung fbel : abidaten.fachbelegungen) {
			...
			fb.idReferenzfach = fbel.idReferenzfach;
			for (int i = 0; i < GostHalbjahr.maxHalbjahre; i++) {
		...
		for (final Sprachbelegung bel : abidaten.sprachendaten.belegungen) {
			final GostLaufbahnplanungExportV2SchuelerSprachbelegung exportBel = new GostLaufbahnplanungExportV2SchuelerSprachbelegung();
			...
			schuelerDaten.sprachbelegungen.add(exportBel);
		}
		for (final Sprachpruefung pruef : abidaten.sprachendaten.pruefungen) {
			final GostLaufbahnplanungExportV2SchuelerSprachpruefung exportPruef = new GostLaufbahnplanungExportV2SchuelerSprachpruefung();
			...
			schuelerDaten.sprachpruefungen.add(exportPruef);
		}
		...
		for (final DTOGostKlausurenVorgaben vorgabe : klausurvorgaben) {
			...
			final GostLaufbahnplanungExportV2GKL gkl = new GostLaufbahnplanungExportV2GKL();
			gkl.id = vorgabe.ID;
			gkl.idFach = vorgabe.Fach_ID;
			gkl.idHalbjahr = vorgabe.Halbjahr.id;
			gkl.quartal = vorgabe.Quartal;
			daten.gkl.add(gkl);
		}
		return daten;
	}
