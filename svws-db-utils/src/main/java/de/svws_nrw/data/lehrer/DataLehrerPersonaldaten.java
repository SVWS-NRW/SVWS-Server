...
public final class DataLehrerPersonaldaten extends DataManagerRevised<Long, DTOLehrer, LehrerPersonaldaten> {
	...
	protected LehrerPersonaldaten map(final DTOLehrer dto) throws ApiOperationException {
		...
		daten.lehraemter.addAll(DataLehrerLehramt.getByLehrerId(conn, dto.ID));
		daten.fachrichtungen.addAll(DataLehrerFachrichtungen.getByLehrerId(conn, dto.ID));
		daten.lehrbefaehigungen.addAll(DataLehrerLehrbefaehigung.getByLehrerId(conn, dto.ID));
		return daten;
	}
