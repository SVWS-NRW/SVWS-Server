 ...
 */
public class DataSchildReportingDatenquelleSchuelerGOStLaufbahnplanungFehler extends DataSchildReportingDatenquelle {
    ...
    List<? extends Object> getDatenInteger(DBEntityManager conn, List<Long> params) {
		...
		ArrayList<SchildReportingSchuelerGOStLaufbahnplanungFehler> result = new ArrayList<>();
		for(Long schuelerID : params) {
					...
					if (ergebnis.fehlercodes.size() > 0) {
						for (GostBelegpruefungErgebnisFehler f : ergebnis.fehlercodes) {
							...
							if (art != GostBelegungsfehlerArt.HINWEIS) {
								SchildReportingSchuelerGOStLaufbahnplanungFehler laufbahnplanungFehler = new SchildReportingSchuelerGOStLaufbahnplanungFehler();
