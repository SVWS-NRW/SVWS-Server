 ...
 */
public final class DataSchildReportingDatenquelleSchuelerGOStHinweise extends DataSchildReportingDatenquelle<SchildReportingGostLaufbahnplanungSchuelerHinweise, Long> {

    /**
     * Erstelle die Datenquelle SchuelerGOStLaufbahnplanungHinweise
     */
	public DataSchildReportingDatenquelleSchuelerGOStHinweise() {
        super(SchildReportingGostLaufbahnplanungSchuelerHinweise.class);
	...
	@Override
	public List<SchildReportingGostLaufbahnplanungSchuelerHinweise> getDaten(final DBEntityManager conn, final List<Long> params) {
		...
		final ArrayList<SchildReportingGostLaufbahnplanungSchuelerHinweise> result = new ArrayList<>();
		for (final Long schuelerID : params) {
					...
					if (!ergebnis.fehlercodes.isEmpty()) {
						for (final GostBelegpruefungErgebnisFehler f : ergebnis.fehlercodes) {
							...
							if (art == GostBelegungsfehlerArt.HINWEIS) {
								final SchildReportingGostLaufbahnplanungSchuelerHinweise laufbahnplanungHinweis = new SchildReportingGostLaufbahnplanungSchuelerHinweise();
