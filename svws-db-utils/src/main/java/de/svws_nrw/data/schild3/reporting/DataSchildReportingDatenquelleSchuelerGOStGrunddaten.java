...
public final class DataSchildReportingDatenquelleSchuelerGOStGrunddaten extends DataSchildReportingDatenquelle<SchildReportingGostLaufbahnplanungSchuelerGrunddaten, Long> {
	...
	@Override
	public List<SchildReportingGostLaufbahnplanungSchuelerGrunddaten> getDaten(final DBEntityManager conn, final List<Long> params) {
		...
		// 1. Informationen der eigenen Schule bestimmen
		...
		final ArrayList<SchildReportingGostLaufbahnplanungSchuelerGrunddaten> result = new ArrayList<>();
		for (final Long schuelerID : params) {
			final SchildReportingGostLaufbahnplanungSchuelerGrunddaten laufbahnplanungGrunddaten = new SchildReportingGostLaufbahnplanungSchuelerGrunddaten();
	 ...
	 */
	private static void eintragBeratungsGOStHalbjahrInLaufbahnplanungGrunddatenErgaenzen(final SchildReportingGostLaufbahnplanungSchuelerGrunddaten laufbahnplanungGrunddaten, final Long schuelerJahrgangID, final Map<Long, DTOJahrgang> jahrgaenge, final int folgeHalbjahr) {
		if (folgeHalbjahr == 2) {
	 ...
	 */
	private static void eintragBeratungslehrkraefteInLaufbahnplanungGrunddatenErgaenzen(final SchildReportingGostLaufbahnplanungSchuelerGrunddaten laufbahnplanungGrunddaten, final DBEntityManager conn, final DTOGostSchueler gostSchueler, final Abiturdaten abiturdaten) {
