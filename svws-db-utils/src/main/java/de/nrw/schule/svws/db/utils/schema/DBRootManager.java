...
public class DBRootManager {
	 ...
	 * 
	 * @param user     der Benutzer der Verbindungen mit den administrativen Rechten
	 */
	...
	private static boolean createDBUser(final DBEntityManager conn, final String nameUser, final String pwUser, final String nameSchema) {
		...
		switch (conn.getDBDriver()) {
			...
			case MSSQL: {
				...
				int c1 = conn.transactionNativeUpdate("CREATE LOGIN " + nameUser + " WITH PASSWORD = '" + pwUser + "'");
	...
	private static boolean grantAdminRights(final DBEntityManager conn, final String nameUser, final String nameSchema) {
		...
		switch (conn.getDBDriver()) {
			...
			case MYSQL: {
				conn.transactionBegin();
				int c1 = conn.transactionNativeUpdate("GRANT ALL PRIVILEGES ON `" + nameSchema + "`.* TO " + nameUser);
				int c2 = conn.transactionNativeUpdate("GRANT GRANT OPTION ON `" + nameSchema + "`.* TO " + nameUser);
				int c3 = conn.transactionNativeUpdate("GRANT CREATE USER ON *.* TO " + nameUser);				
				return conn.transactionCommit() && (c1 > Integer.MIN_VALUE) && (c2 > Integer.MIN_VALUE) && (c3 > Integer.MIN_VALUE);
			}
			case MSSQL: {
				conn.transactionBegin();
				int c1 = conn.transactionNativeUpdate("USE [" + nameSchema + "]");
				int c2 = conn.transactionNativeUpdate("CREATE USER " + nameUser + " FOR LOGIN " + nameUser);
				int c3 = conn.transactionNativeUpdate("GRANT ALL PRIVILEGES ON DATABASE::[" + nameSchema + "] TO " + nameUser + " WITH GRANT OPTION");
				int c4 = conn.transactionNativeUpdate("GRANT CONTROL ON DATABASE::[" + nameSchema + "] TO " + nameUser + " WITH GRANT OPTION");
	...
	public boolean dropDBUserIfExists(final String nameUser) {
		...
		switch (conn.getDBDriver()) {
			...
			case MSSQL:
				conn.transactionBegin();
				int c1 = conn.transactionNativeDelete("DROP USER IF EXISTS " + nameUser);
				int c2 = conn.transactionNativeDelete("DROP LOGIN " + nameUser);
	 ...
	 */
	private static boolean removeDBFile(DBDriver driver, String db_location) {
		switch (driver) {
			case MARIA_DB:
			case MYSQL:
			case MSSQL:
