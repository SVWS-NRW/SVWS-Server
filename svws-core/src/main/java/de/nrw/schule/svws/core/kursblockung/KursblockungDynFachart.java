...
public class KursblockungDynFachart {
	...
	/**
	 * Eine StringDarstellung der Fachart für Warnungen und Benutzerfehler, beispielsweise 'D;LK'.
	 */
	private final @NotNull String representation;

	/**
	 * Ein Array aller Kurse dieser Fachart. Das Array bleibt dynamisch sortiert, so dass im Array zunächst der Kurs mit
	 * der geringsten Schüleranzahl ist.
	 */
	 ...
	 */
	public KursblockungDynFachart(@NotNull Random pRandom, int pNr, @NotNull String pRepresentation,
			@NotNull KursblockungDynStatistik pStatistik) {
		_random = pRandom;
		nr = pNr;
	...
	KursblockungDynKurs gibKleinstenKursInSchiene(int pSchiene, @NotNull boolean[] kursGesperrt) {
		for (int i = 0; i < kursArr.length; i++) {
			...
			@NotNull
			KursblockungDynKurs kurs = kursArr[i];
	...
	boolean gibHatMultikurs() {
		for (@NotNull
		KursblockungDynKurs kurs : kursArr) {
	...
	public void aktionSchuelerWurdeHinzugefuegt() {
		...
		for (int i = 1; i < kursArr.length; i++) {
			@NotNull
			KursblockungDynKurs kursL = kursArr[i - 1];
			@NotNull
			KursblockungDynKurs kursR = kursArr[i];
			boolean b1 = kursL.gibSchuelerAnzahl() > kursR.gibSchuelerAnzahl();
			boolean b2 = (kursL.gibSchuelerAnzahl() == kursR.gibSchuelerAnzahl()) && (kursL.gibID() > kursR.gibID());
			if (b1 || b2) {
				kursArr[i - 1] = kursR;
				kursArr[i] = kursL;
			}
	...
	void aktionZufaelligerKursWandertNachSchiene(int pSchiene) {
		@NotNull
		int[] perm = KursblockungStatic.gibPermutation(_random, kursArr.length);
