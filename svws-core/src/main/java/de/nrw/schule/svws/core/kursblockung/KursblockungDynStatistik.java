...
public class KursblockungDynStatistik {
	...
	void debug(@NotNull String pPrefix) {
		System.out.println(pPrefix + ", NW = " + bewertungNichtwahlen + ", FW = " + bewertungFachartPaar + ", KDs = "
				+ bewertungKursdifferenzenMaxIndex + " = " + Arrays.toString(bewertungKursdifferenzen));
	}
	...
	int gibBewertungZustandS_NW_KD() {
		...
		for (int i = bewertungKursdifferenzen.length - 1; i >= 0; i--) {
			...
			// Kursdifferenz schlechter?
			if (bewertungKursdifferenzen[i] > bewertungKursdifferenzenSaveS[i]) {
				return -1;
			}
			// Kursdifferenz besser?
			if (bewertungKursdifferenzen[i] < bewertungKursdifferenzenSaveS[i]) {
				return +1;
			}
		}
	...
	void aktionInitialisiere(@NotNull int @NotNull [][] pMatrixFachartPaar, int pMaxSchueler, int pMaxFacharten) {
		...
		// Es muss gespeichert werden, da sonst beim Laden vor einem Speichern
		// ein ungültiger Zustand entsteht.
	...
	void aktionKursdifferenzEntfernen(int pIndex) {
		...
		if (pIndex == bewertungKursdifferenzenMaxIndex)
			while ((bewertungKursdifferenzen[bewertungKursdifferenzenMaxIndex] == 0)
					&& (bewertungKursdifferenzenMaxIndex > 0))
	...
	void aktionBewertungSpeichernS() {
		...
		System.arraycopy(bewertungKursdifferenzen, 0, bewertungKursdifferenzenSaveS, 0,
				bewertungKursdifferenzen.length);
	}
	...
	void aktionBewertungSpeichernK() {
		...
		System.arraycopy(bewertungKursdifferenzen, 0, bewertungKursdifferenzenSaveK, 0,
				bewertungKursdifferenzen.length);
	}
	...
	void aktionBewertungSpeichernG() {
		...
		System.arraycopy(bewertungKursdifferenzen, 0, bewertungKursdifferenzenSaveG, 0,
				bewertungKursdifferenzen.length);
	}
