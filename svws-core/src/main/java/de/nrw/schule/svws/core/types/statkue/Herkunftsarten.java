...
public enum Herkunftsarten {
	...
	private static final @NotNull HashMap<@NotNull String, @NotNull Herkunftsarten> _mapKuerzel = new HashMap<>();

	/** Eine Map mit der Zuordnung der Herkunftsart zu der ID der Herkunftsart */
	private static final @NotNull HashMap<@NotNull Long, @NotNull Herkunftsarten> _mapID = new HashMap<>();
	...
	private Herkunftsarten(@NotNull HerkunftsartKatalogEintrag@NotNull[] historie) {
		...
		for (int i = 0; i < historie.length; i++) {
			...
			for (@NotNull HerkunftsartKatalogEintragBezeichnung bez : historie[i].bezeichnungen) {
	 ...
	 */
	private static @NotNull HashMap<@NotNull String, @NotNull Herkunftsarten> getMapHerkunftsartByKuerzel() {
	 ...
	 */
	private static @NotNull HashMap<@NotNull Long, @NotNull Herkunftsarten> getMapHerkunftsartByID() {
		if (_mapID.size() == 0)
			for (Herkunftsarten j : Herkunftsarten.values()) {
				for (HerkunftsartKatalogEintrag k : j.historie)
					_mapID.put(k.id, j);
			}
		return _mapID;
	}
	...
	public String getBezeichnung(Schulform schulform) {
		...
		if (daten.bezeichnungen != null) {
			for (int i = 0; i < daten.bezeichnungen.size(); i++) {
				HerkunftsartKatalogEintragBezeichnung bez = daten.bezeichnungen.get(i);
	 ...
	 */
	public static @NotNull List<@NotNull Herkunftsarten> get(Schulform schulform) {
		@NotNull Vector<@NotNull Herkunftsarten> result = new Vector<>();
		if (schulform == null)
			return result;
		@NotNull Herkunftsarten@NotNull[] herkunftsarten = Herkunftsarten.values();
		for (int i = 0; i < herkunftsarten.length; i++) {
			@NotNull Herkunftsarten herkunftsart = herkunftsarten[i];
