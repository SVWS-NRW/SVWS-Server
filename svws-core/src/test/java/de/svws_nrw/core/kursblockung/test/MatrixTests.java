...
public class MatrixTests {
	...
	void test001() {
		...
		KursblockungMatrix m = new KursblockungMatrix(rnd, 5, 6);
		long[][] d = m.getMatrix();
		...
		int[] r2c = m.gibMinimalesBipartitesMatchingGewichtet(false);
	...
	void test002() {
		...
		for (int dimR = 1; dimR <= 30; dimR++) {
			for (int dimC = 1; dimC <= 30; dimC++) {
				KursblockungMatrix m = new KursblockungMatrix(rnd, dimR, dimC);
				long[][] d = m.getMatrix();
